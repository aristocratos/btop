...
3. **Compile**
   ...
   | `USE_LEVELZERO=true` | Use Level Zero instead of the legacy PMU backend for Intel GPUs     |
...
3. **Compile**
   ...
   | `-DUSE_LEVELZERO=<ON\|OFF>` | Use Level Zero instead of the legacy Intel PMU backend (OFF by default) |
