...
namespace Term {
    ...
    const string mouse_on =
        Fx::e + "?1002h" + Fx::e + "?1015h" + Fx::e + "?1006h"; //? Enable reporting of mouse position on click and release
...
namespace Tools {
    ...
    const T& safeVal(
        ...
        if (auto it = map.find(key); it != map.end()) {
            ...
            Logger::error(
                fmt::format("safeVal() called with invalid key: [{}] in file: {} on line: {}", key, loc.file_name(), loc.line())
            );
            return fallback;
        }
    };
    ...
    const T& safeVal(const std::unordered_map<K, T>& map, const K& key, const T& fallback = T {}) {
        if (auto it = map.find(key); it != map.end()) {
            ...
            Logger::error(
                fmt::format(
                    "safeVal() called with invalid key: [{}] (Compile btop with DEBUG=true for more extensive logging!)", key
                )
            );
            return fallback;
        }
    ...
    const T& safeVal(
        ...
        if (index < vec.size()) {
            ...
            Logger::error(
                fmt::format(
                    "safeVal() called with invalid index: [{}] in file: {} on line: {}", index, loc.file_name(), loc.line()
                )
            );
            return fallback;
        }
    ...
    const T& safeVal(const std::vector<T>& vec, const size_t& index, const T& fallback = T {}) {
        if (index < vec.size()) {
            ...
            Logger::error(
                fmt::format(
                    "safeVal() called with invalid index: [{}] (Compile btop with DEBUG=true for more extensive logging!)", index
                )
            );
            return fallback;
        }
