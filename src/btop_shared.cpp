...
namespace Proc {
    ...
    auto matches_filter(const proc_info& proc, const std::string& filter) -> bool {
        ...
        return std::to_string(proc.pid).contains(filter) || s_contains_ic(proc.name, filter) || s_contains_ic(proc.cmd, filter) ||
    ...
    void _collect_prefixes(tree_proc& t, const bool is_last, const string& header) {
        ...
        for (auto child = t.children.begin(); child != t.children.end(); ++child) {
            _collect_prefixes(*child, child == (t.children.end() - 1), is_filtered ? "" : header + (is_last ? "   " : " │ "));
        }
    }
} // namespace Proc
