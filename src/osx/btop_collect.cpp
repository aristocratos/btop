...
IOReportSubscriptionRef IOReportCreateSubscription(
	void* a, CFMutableDictionaryRef b, CFMutableDictionaryRef* c, uint64_t d, CFTypeRef cfnull
);
CFDictionaryRef IOReportCreateSamples(IOReportSubscriptionRef sub, CFMutableDictionaryRef chan, CFTypeRef cfnull);
...
namespace Gpu {
	...
	namespace AppleSilicon {
		...
		static string cfstring_to_string(CFStringRef cfstr) {
			if (not cfstr) return "";
			char buf[256];
			if (CFStringGetCString(cfstr, buf, sizeof(buf), kCFStringEncodingUTF8)) return string(buf);
			return "";
		}
		...
		static string get_chip_name() {
			...
			if (sysctlbyname("machdep.cpu.brand_string", buf, &size, nullptr, 0) == 0) return string(buf);
		...
		static void get_gpu_freqs_from_pmgr() {
			...
			if (IOServiceGetMatchingServices(kIOMainPortDefault, matchDict, &iter_raw) != kIOReturnSuccess) return;
			IORef iter(iter_raw);
			...
			while ((entry_raw = IOIteratorNext(iter)) != 0) {
				...
				if (IORegistryEntryGetName(entry, name) == kIOReturnSuccess and string(name) == "pmgr") {
					...
					if (IORegistryEntryCreateCFProperties(entry, &props_raw, kCFAllocatorDefault, 0) ==
							kIOReturnSuccess and
						props_raw) {
		...
		bool init() {
			...
			CFRef<CFStringRef> gpu_stats_group(
				CFStringCreateWithCString(kCFAllocatorDefault, "GPU Stats", kCFStringEncodingUTF8)
			);
			CFRef<CFStringRef> gpu_perf_subgroup(
				CFStringCreateWithCString(kCFAllocatorDefault, "GPU Performance States", kCFStringEncodingUTF8)
			);
			CFRef<CFStringRef> energy_group(
...
#	if __MAC_OS_X_VERSION_MIN_REQUIRED > 101504
			...
			CFRef<CFDictionaryRef> match(CFDictionaryCreate(
				nullptr, (const void**)keys, values, 2, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks
			));
			...
			if (gpu_temp_count > 0) return static_cast<long long>(round(gpu_temp_sum / gpu_temp_count));
#	endif
			return -1;
		}
		...
		bool collect(gpu_info* gpus_slice) {
			...
			if constexpr (is_init) {
				...
				if (sysctlbyname("hw.memsize", &memsize, &size, nullptr, 0) == 0) gpus_slice[0].mem_total = memsize;
			...
			for (long i = 0; i < chan_count; i++) {
				...
				if (group == "GPU Stats" and subgroup == "GPU Performance States" and channel == "GPUPH") {
					...
					for (int32_t s = 0; s < state_count; s++) {
						...
						if (name == "IDLE" or name == "OFF" or name == "DOWN") offset = s + 1;
					...
					if (total_residency > 0) {
						double usage_ratio =
							static_cast<double>(active_residency) / static_cast<double>(total_residency);
				...
				gpus_slice[0]
					...
					.push_back(clamp(
						static_cast<long long>(round(
							static_cast<double>(gpus_slice[0].pwr_usage) * 100.0 /
							static_cast<double>(gpus_slice[0].pwr_max_usage)
						)),
						0ll,
						100ll
					));
			...
			if (gpus_slice[0].supported_functions.temp_info and Config::getB("check_temp")) {
				...
				if (temp > 0) gpus_slice[0].temp.push_back(temp);
			}
				...
				if (host_statistics64(mach_port, HOST_VM_INFO64, (host_info64_t)&vm_stats, &count) == KERN_SUCCESS) {
					long long used =
						(static_cast<int64_t>(vm_stats.active_count) + static_cast<int64_t>(vm_stats.inactive_count) +
						 static_cast<int64_t>(vm_stats.wire_count) + static_cast<int64_t>(vm_stats.speculative_count) +
						 static_cast<int64_t>(vm_stats.compressor_page_count) -
						 static_cast<int64_t>(vm_stats.purgeable_count) -
						 static_cast<int64_t>(vm_stats.external_page_count)) *
						static_cast<int64_t>(page_size);
					if (used < 0) used = 0;
					gpus_slice[0].mem_used = used;
					if (gpus_slice[0].mem_total > 0) {
						auto used_pct = static_cast<long long>(
							round(static_cast<double>(used) * 100.0 / static_cast<double>(gpus_slice[0].mem_total))
						);
	...
	auto collect(bool no_update) -> vector<gpu_info>& {
		...
		for (auto& gpu : gpus) {
			...
			if (gpu.supported_functions.pwr_usage) pwr_total += gpu.pwr_usage;
		...
		if (not gpus.empty()) {
			...
			if (mem_total != 0) shared_gpu_percent.at("gpu-vram-total").push_back(mem_usage_total * 100 / mem_total);
		...
		if (width != 0) {
			...
			while (cmp_greater(shared_gpu_percent.at("gpu-vram-total").size(), width))
				shared_gpu_percent.at("gpu-vram-total").pop_front();
			while (cmp_greater(shared_gpu_percent.at("gpu-pwr-total").size(), width))
...
class MachProcessorInfo {
  public:
	...
	virtual ~MachProcessorInfo() {
		vm_deallocate(
			mach_task_self(), (vm_address_t)info_array, (vm_size_t)sizeof(processor_info_array_t) * info_count
		);
	}
};
...
namespace Shared {
	...
	void init() {
		...
		if (mach_timebase_info(&convf) == KERN_SUCCESS) {
			...
			Logger::warning(
				"Could not get mach clock tick conversion factor. Defaulting to 100, processes cpu usage might be "
				"incorrect."
			);
			machTck = 100;
		}
...
namespace Cpu {
	...
	bool get_sensors() {
		Logger::debug(
			"get_sensors(): show_coretemp={} check_temp={}", Config::getB("show_coretemp"), Config::getB("check_temp")
		);
		got_sensors = false;
		...
		try {
			if (macM1) {
				...
				for (int core = 0; core < Shared::coreCount; core++) {
					long long temp = smcCon.getTemp(
						(core / threadsPerCore) + core_offset
					); // same temp for all threads of same physical core
	...
	auto collect(bool no_update) -> cpu_info& {
		...
		for (i = 0; i < cpu_count; i++) {
			...
			for (int x = 0;
				 const unsigned int c_state : {CPU_STATE_USER, CPU_STATE_NICE, CPU_STATE_SYSTEM, CPU_STATE_IDLE}) {
		...
		for (int ii = 0; const auto& val : times_summed) {
			cpu.cpu_percent.at(time_names.at(ii))
				.push_back(clamp(
					(long long)round((double)(val - cpu_old.at(time_names.at(ii))) * 100 / calc_totals), 0ll, 100ll
				));
...
namespace Mem {
	...
	bool isWhole(io_registry_entry_t volumeRef) {
		CFBooleanRef isWhole =
			(CFBooleanRef)IORegistryEntryCreateCFProperty(volumeRef, CFSTR("Whole"), kCFAllocatorDefault, 0);
			...
			if (volumeRef) {
				if (!isWhole(volumeRef)) {
					...
					if (!mapping.contains(device)) {
						device = "/dev/" + bsdName; // try again with BSD name - not all volumes seem to have
													// VolGroupMntFromName property
					}
					if (device != "") {
						if (mapping.contains(device)) {
							...
							if (disks.contains(mountpoint)) {
								...
								IORegistryEntryCreateCFProperties(
									volumeRef, (CFMutableDictionaryRef*)&properties, kCFAllocatorDefault, 0
								);
								if (properties) {
									...
									if (statistics) {
										disk_ios++;
										int64_t readBytes =
											getCFNumber(statistics, CFSTR("Bytes read from block device"));
										if (disk.io_read.empty()) disk.io_read.push_back(0);
										else disk.io_read.push_back(max((int64_t)0, (readBytes - disk.old_io.at(0))));
										disk.old_io.at(0) = readBytes;
										while (cmp_greater(disk.io_read.size(), width * 2))
											disk.io_read.pop_front();

										int64_t writeBytes =
											getCFNumber(statistics, CFSTR("Bytes written to block device"));
										if (disk.io_write.empty()) disk.io_write.push_back(0);
										else disk.io_write.push_back(max((int64_t)0, (writeBytes - disk.old_io.at(1))));
										disk.old_io.at(1) = writeBytes;
										while (cmp_greater(disk.io_write.size(), width * 2))
											disk.io_write.pop_front();

										// IOKit does not give us IO times, (use IO read + IO write with 1 MiB being
										// 100% to get some activity indication)
										if (disk.io_activity.empty()) disk.io_activity.push_back(0);
										else
											disk.io_activity.push_back(clamp(
												(long)round(
													(double)(disk.io_write.back() + disk.io_read.back()) / (1 << 20)
												),
												0l,
												100l
											));
	...
	auto collect(bool no_update) -> mem_info& {
		...
		if (show_disks) {
			...
			for (const auto& name : last_found)
				if (not is_in(name, "/", "swap", "/dev")) mem.disks_order.push_back(name);
			...
			if (sysctl(mib, 4, processes.get(), &size, nullptr, 0) == 0) {
				...
				for (size_t i = 0; i < count; i++) { //* iterate over all processes in kinfo_proc
					...
					if (no_cache) {
								...
								if (size_t null_pos = proc_args.find('\0', sizeof(argc)); null_pos != string::npos) {
									if (size_t start_pos = proc_args.find_first_not_of('\0', null_pos);
										...
										while (argc-- > 0 and null_pos != string::npos and
											...
											new_proc.cmd +=
												(string)proc_args.substr(start_pos, null_pos - start_pos) + ' ';
						...
						new_proc.cpu_s =
							kproc.kp_proc.p_starttime.tv_sec * 1'000'000 + kproc.kp_proc.p_starttime.tv_usec;
					...
					new_proc.cpu_p = clamp(
						round(
							((cpu_t - new_proc.cpu_t) * Shared::machTck) / ((cputimes - old_cputimes) * Shared::clkTck)
						) * cmult /
							1000.0,
						0.0,
