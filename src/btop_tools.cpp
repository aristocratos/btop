...
namespace Tools {
	...
	auto ssplit(const string& str, const char& delim) -> vector<string> {
		vector<string> out;
		for (const auto& s : str | rng::views::split(delim) | rng::views::transform([](auto&& rng) {
	...
	atomic_lock::~atomic_lock() noexcept { this->atom.store(false); }
	...
	bool DebugTimer::is_running() { return running; }
} // namespace Tools
	...
	void log_write(const Level level, const std::string_view msg) {
		...
		try {
			...
			if (fs::exists(log_file) and fs::file_size(log_file, ec) > 1024 << 10 and not ec) {
				...
				if (not ec) fs::rename(log_file, old_log, ec);
			}
			if (not ec) {
