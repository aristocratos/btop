...
namespace Term {
    ...
    auto get_min_size(const string& boxes) -> array<int, 2> {
        ...
        if (cpu and width < Cpu::min_width)
            width = Cpu::min_width;
#ifdef GPU_SUPPORT
        if (gpu != 0 and width < Gpu::min_width)
            width = Gpu::min_width;
#endif
...
namespace Tools {
    ...
    string sec_to_dhms(size_t seconds, bool no_days, bool no_seconds) {
        ...
        seconds %= 60;
        string out = (not no_days and days > 0 ? to_string(days) + "d " : "") + (hours < 10 ? "0" : "") + to_string(hours) + ':' +
    ...
    string floating_humanizer(uint64_t value, bool shorten, size_t start, bool bit, bool per_second) {
        ...
        static const array mebiUnits_byte { "Byte"s, "KiB"s, "MiB"s, "GiB"s, "TiB"s, "PiB"s,
                                            "EiB"s,  "ZiB"s, "YiB"s, "BiB"s, "GEB"s };
        static const array megaUnits_bit { "bit"s, "Kb"s, "Mb"s, "Gb"s, "Tb"s, "Pb"s, "Eb"s, "Zb"s, "Yb"s, "Bb"s, "Gb"s };
        static const array megaUnits_byte { "Byte"s, "KB"s, "MB"s, "GB"s, "TB"s, "PB"s, "EB"s, "ZB"s, "YB"s, "BB"s, "GB"s };
    ...
    void DebugTimer::report() {
        ...
        if (start_time == 0 and elapsed_time == 0)
            ...
            report_line =
                fmt::format(custom_locale, "Timer [{}] (running) currently at {:L} μs", name, time_micros() - start_time);
        else
