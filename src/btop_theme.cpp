...
namespace Theme {
    ...
    const std::unordered_map<string, string> Default_theme = { { "main_bg", "#00" },
                                                               { "main_fg", "#cc" },
                                                               { "title", "#ee" },
                                                               { "hi_fg", "#b54040" },
                                                               { "selected_bg", "#6a2f2f" },
    ...
    string dec_to_color(int r, int g, int b, bool t_to_256, const string& depth) {
        ...
        b = std::clamp(b, 0, 255);
        if (t_to_256)
            return pre + to_string(truecolor_to_256(r, g, b)) + "m";
        else
            return pre + to_string(r) + ";" + to_string(g) + ";" + to_string(b) + "m";
    }
    ...
    namespace {
        ...
        array<int, 3> hex_to_dec(string hexa) {
            if (hexa.size() > 1) {
                ...
                if (hexa.size() == 2) {
                    ...
                    return array { stoi(hexa.substr(0, 2), nullptr, 16),
                                   stoi(hexa.substr(2, 2), nullptr, 16),
                                   stoi(hexa.substr(4, 2), nullptr, 16) };
                }
            }
        ...
        void generateGradients() {
            ...
            for (const auto& [name, source_arr] : rgbs) {
                if (not name.ends_with("_start"))
                    continue;
                const string color_name { rtrim(name, "_start") };

                //? input_colors[start,mid,end][red,green,blue]
                const array<array<int, 3>, 3> input_colors = { source_arr, rgbs[color_name + "_mid"], rgbs[color_name + "_end"] };
                ...
                if (input_colors[2][0] >= 0) {
                    ...
                    //? Split iteration in two passes of 50 + 51 instead of one pass of 101 if gradient has start, mid and end
                    // values defined
        ...
        void generateTTYColors() {
            ...
            for (const auto& c : colors) {
                if (not c.first.ends_with("_start"))
                    continue;
