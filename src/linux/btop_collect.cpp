	...
	auto collect(bool no_update) -> mem_info& {
		...
		if (show_disks) {
			...
			try {
				...
				auto only_first_btrfs_subvol = Config::getB("only_first_btrfs_subvol");
				...
				if (diskread.good()) {
					...
					while (not diskread.eof()) {
						...
						if (only_first_btrfs_subvol and fstype == "btrfs") {
							string devname = fs::canonical(dev, ec).filename();
							if (!found_btrfs_subvolumes.insert(devname).second) continue;
						}
