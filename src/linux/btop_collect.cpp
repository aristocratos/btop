...
#    define NVML_PCIE_UTIL_RX_BYTES 1
        ...
        typedef int nvmlReturn_t,   // enums are basically ints
            nvmlTemperatureThresholds_t, nvmlClockType_t, nvmlPstates_t, nvmlTemperatureSensors_t, nvmlPcieUtilCounter_t;
...
namespace Cpu {
    ...
    std::unordered_map<string, long long> cpu_old = { { "totals", 0 },  { "idles", 0 }, { "user", 0 },   { "nice", 0 },
            ...
            if (cpuinfo.bad())
                ...
                if (not name.empty()) {
                    ...
                    if (name_vec.size() < 2)
                        ...
                        return capitalize(name_vec.at(1)) + (name_vec.size() > 2 ? ' ' + capitalize(name_vec.at(2)) : "");
                }
            }
        ...
        try {
            ...
            if (fs::exists(fs::path("/sys/class/hwmon")) and access("/sys/class/hwmon", R_OK) != -1) {
                for (const auto& dir : fs::directory_iterator(fs::path("/sys/class/hwmon"))) {
                    ...
                    if (v_contains(search_paths, add_path) or v_contains(search_paths, add_path / "device"))
                        continue;
    ...
    auto get_battery() -> tuple<int, float, long, string> {
        ...
        auto& b =
            (battery_sel != "Auto" and batteries.contains(battery_sel) ? batteries.at(battery_sel) : batteries.at(auto_sel));
        ...
        try {
            ...
            for (; i <= target or (cread.good() and cread.peek() == 'c'); i++) {
                ...
                if ((not cread.good() or cread.peek() != 'c') and i <= target) {
                    ...
                    //? Expected on kernel 2.6.3> : 0=user, 1=nice, 2=system, 3=idle, 4=iowait, 5=irq, 6=softirq, 7=steal,
                    ...
                    const long long totals =
                        max(0ll, total_sum - (times.size() > 8 ? std::accumulate(times.begin() + 8, times.end(), 0ll) : 0));
...
#    undef LOAD_SYM
            ...
            if (result != NVML_SUCCESS) {
                Logger::debug(
                    std::string("Failed to initialize NVML, NVIDIA GPUs will not be detected: ") + nvmlErrorString(result)
                );
                return false;
            }
        ...
        bool collect(gpu_info* gpus_slice) { // raw pointer to vector data, size == device_count
            ...
            for (unsigned int i = 0; i < device_count; ++i) {
                ...
                if (gpus_slice[i].supported_functions.pwr_usage) {
                    ...
                    if (result != NVML_SUCCESS) {
                        ...
                        gpus_slice[i]
                            ...
                            .push_back(clamp(
                                (long long)round((double)gpus_slice[i].pwr_usage * 100.0 / (double)gpus_slice[i].pwr_max_usage),
                                0ll,
                                100ll
                            ));
                    }
                ...
                if constexpr (is_init) { // there doesn't seem to be a better way to do this, but this should be fine considering
                                         // it's just 2 lines
                    pcie_tx_thread.join();
                    pcie_rx_thread.join();
...
#    endif
        ...
        template<bool is_init>
        bool
        collect(gpu_info* gpus_slice) { // raw pointer to vector data, size == device_count, offset by Nvml::device_count elements
            if (!initialized)
                return false;
    ...
    namespace Intel {
        ...
        bool collect(gpu_info* gpus_slice) {
            ...
            gpus_slice->gpu_percent.at("gpu-pwr-totals")
                .push_back(
                    clamp((long long)round((double)gpus_slice->pwr_usage * 100.0 / (double)gpus_slice->pwr_max_usage), 0ll, 100ll)
                );
...
static auto convert_ascii_escapes(const std::string& input) -> std::string {
    ...
    for (std::size_t i = 0; i < input.size(); ++i) {
        if (input[i] == '\\' &&
            ...
            i + 3 < input.size() && std::isdigit(input[i + 1]) && std::isdigit(input[i + 2]) && std::isdigit(input[i + 3])) {
            ...
            try {
                ...
                if (diskread.good()) {
                    ...
                    while (not diskread.eof()) {
                        ...
                        if (v_contains(ignore_list, mountpoint) or v_contains(found, mountpoint))
                            continue;
                        ...
                        if (fstype == "zfs" && (zfs_dataset_name_start = dev.find('/')) != std::string::npos && zfs_hide_datasets)
                            continue;
                        ...
                        if (disk.fstype == "zfs") {
                            ...
                            if (uptime == old_uptime || disk.io_activity.empty())
                                ...
                                disk.io_activity.push_back(clamp(
                                    (long)round((double)(io_ticks - disk.old_io.at(2)) / (uptime - old_uptime) / 10), 0l, 100l
                                ));
                            disk.old_io.at(2) = io_ticks;
                            while (cmp_greater(disk.io_activity.size(), width * 2))
                                disk.io_activity.pop_front();
            ...
            for (const auto& iface : interfaces) {
                ...
                for (const string dir : { "download", "upload" }) {
                    ...
                    if (net_auto and selected_iface == iface) {
                        if (net_sync and saved_stat.speed < netif.stat.at(dir == "download" ? "upload" : "download").speed)
                            continue;
    ...
    auto collect(bool no_update) -> vector<proc_info>& {
        ...
        //* ---------------------------------------------Collection start----------------------------------------------
            ...
            if (should_filter_kernel and ++proc_clear_count >= 256) {
                ...
                //? In that event processes that acquire old kernel pids would also be filtered out so we need to manually clean
