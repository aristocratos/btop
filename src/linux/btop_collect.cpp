	...
	try {
	  ...
	  if (not search_paths.empty()) {
		for (const auto& path : search_paths) {
		  ...
		  for (const auto& file : fs::directory_iterator(path)) {
			...
			const int64_t high =
			    stol(readfile(fs::path(basepath + "max"), "80000")) / 1000;
			const int64_t crit =
			    stol(readfile(fs::path(basepath + "crit"), "95000")) / 1000;

			found_sensors[sensor_name] = {fs::path(basepath + "input"), label,
			                              temp, high, crit};
	  ...
	  if (not got_cpu and fs::exists(fs::path("/sys/class/thermal"))) {
		...
		for (int i = 0; fs::exists(fs::path(rootpath + to_string(i))); i++) {
		  ...
		  int64_t high, crit;
		  for (int ii = 0; fs::exists(
		           basepath / string("trip_point_" + to_string(ii) + "_temp"));
		       ii++) {
			const string trip_type = readfile(
			    basepath / string("trip_point_" + to_string(ii) + "_type"));
			if (not is_in(trip_type, "high", "critical")) continue;
			auto& val = (trip_type == "high" ? high : crit);
			val = stol(readfile(basepath / string("trip_point_" +
			                                      to_string(ii) + "_temp"),
			                    "0")) /
			      1000;
		  }
		  ...
		  found_sensors[sensor_name] = {basepath / "temp", label, temp, high,
		                                crit};
		}
	  }
  ...
  auto collect(const bool no_update) -> cpu_info& {
	...
	try {
	  ...
	  for (int i = 0; cread.good() and cread.peek() == 'c'; i++) {
		...
		const long long totals = max(
		    0ll, total_sum -
		             (times.size() > 8
		                  ? std::accumulate(times.begin() + 8, times.end(), 0)
		                  : 0));
			  ...
			  if (not disks.contains(mountpoint)) {
				...
				while (devname.size() >= 2) {
				  if (fs::exists("/sys/block/" + devname + "/stat", ec) and
				      access(string("/sys/block/" + devname + "/stat").c_str(),
				             R_OK) == 0) {
					if (c > 0 and
					    fs::exists(
					        "/sys/block/" + devname + '/' +
  ...
  void _tree_gen(proc_info& cur_proc, vector<proc_info>& in_procs,
	...
	if (not collapsed and not filtering) {
	  ...
	  if (cur_proc.short_cmd.empty() and not cur_proc.cmd.empty()) {
		...
		cmd_view = cmd_view.substr((size_t)0,
		                           min(cmd_view.find(' '), cmd_view.size()));
	...
	if (sorted_change or not no_update) {
	  if (reverse) {
		switch (v_index(sort_vector, sorting)) {
		  case 0:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::pid);
			break;
		  case 1:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::name);
			break;
		  case 2:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::cmd);
			break;
		  case 3:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::threads);
			break;
		  case 4:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::user);
			break;
		  case 5:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::mem);
			break;
		  case 6:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::cpu_p);
			break;
		  case 7:
			rng::stable_sort(current_procs, rng::less{}, &proc_info::cpu_c);
			break;
		}
	  } else {
		switch (v_index(sort_vector, sorting)) {
		  case 0:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::pid);
			break;
		  case 1:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::name);
			break;
		  case 2:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::cmd);
			break;
		  case 3:
			...
			break;
		  case 4:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::user);
			break;
		  case 5:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::mem);
			break;
		  case 6:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::cpu_p);
			break;
		  case 7:
			rng::stable_sort(current_procs, rng::greater{}, &proc_info::cpu_c);
			break;
		}
	  }
	  ...
	  if (not tree and not reverse and sorting == "cpu lazy") {
		...
		for (size_t i = 0, x = 0, offset = 0; i < current_procs.size(); i++) {
		  ...
		  if (i == offset and current_procs.at(i).cpu_p > 30.0)
			...
			rotate(current_procs.begin() + offset, current_procs.begin() + i,
