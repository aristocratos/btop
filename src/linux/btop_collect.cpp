...
typedef void *nvmlDevice_t; // we won't be accessing any of the underlying
                            // struct's properties, so this is fine
typedef int nvmlReturn_t,   // enums are basically ints
    nvmlTemperatureThresholds_t, nvmlClockType_t, nvmlPstates_t,
    nvmlTemperatureSensors_t, nvmlPcieUtilCounter_t;
...
nvmlReturn_t (*nvmlDeviceGetTemperatureThreshold)(nvmlDevice_t,
                                                  ...
                                                  unsigned int *);
nvmlReturn_t (*nvmlDeviceGetUtilizationRates)(nvmlDevice_t,
                                              nvmlUtilization_t *);
...
typedef int rsmi_status_t, rsmi_temperature_metric_t, rsmi_clk_type_t,
    rsmi_memory_type_t;
...
rsmi_status_t (*rsmi_dev_temp_metric_get)(uint32_t, uint32_t,
                                          rsmi_temperature_metric_t, int64_t *);
...
rsmi_status_t (*rsmi_dev_memory_total_get)(uint32_t, rsmi_memory_type_t,
                                           uint64_t *);
rsmi_status_t (*rsmi_dev_memory_usage_get)(uint32_t, rsmi_memory_type_t,
                                           uint64_t *);
...
void init() {
  ...
  procPath =
      (fs::is_directory(fs::path("/proc")) and access("/proc", R_OK) != -1)
          ? "/proc"
          : "";
  if (procPath.empty())
    throw std::runtime_error(
        "Proc filesystem not found or no permission to read from it!");

  passwd_path = (fs::is_regular_file(fs::path("/etc/passwd")) and
                 access("/etc/passwd", R_OK) != -1)
                    ? "/etc/passwd"
                    : "";
  if (passwd_path.empty())
    Logger::warning(
        "Could not read /etc/passwd, will show UID instead of username.");
  ...
  if (pageSize <= 0) {
    pageSize = 4096;
    Logger::warning("Could not get system page size. Defaulting to 4096, "
                    "processes memory usage might be incorrect.");
  }
  ...
  if (clkTck <= 0) {
    ...
    Logger::warning("Could not get system clock ticks per second. Defaulting "
                    "to 100, processes cpu usage might be incorrect.");
  }

  //? Init for namespace Cpu
  Cpu::current_cpu.core_percent.insert(Cpu::current_cpu.core_percent.begin(),
                                       Shared::coreCount, {});
  Cpu::current_cpu.temp.insert(Cpu::current_cpu.temp.begin(),
                               Shared::coreCount + 1, {});
  Cpu::core_old_totals.insert(Cpu::core_old_totals.begin(), Shared::coreCount,
                              0);
  Cpu::core_old_idles.insert(Cpu::core_old_idles.begin(), Shared::coreCount, 0);

  for (int i = 0; i < Shared::coreCount; ++i) {
    Cpu::core_freq.push_back("/sys/devices/system/cpu/cpufreq/policy" +
                             to_string(i) + "/scaling_cur_freq");
    if (not fs::exists(Cpu::core_freq.back()) or
        access(Cpu::core_freq.back().c_str(), R_OK) == -1) {
      Cpu::core_freq.pop_back();
    }
  }

  Cpu::collect();
  if (Runner::coreNum_reset)
    Runner::coreNum_reset = false;
  ...
  if (not Gpu::gpu_names.empty()) {
    ...
    for (size_t i = 0; i < gpu_b_height_offsets.size(); ++i)
      gpu_b_height_offsets[i] =
          ...
          (gpus[i].supported_functions.mem_total or
...
const array time_names{"user"s, "nice"s,    "system"s, "idle"s,  "iowait"s,
                       "irq"s,  "softirq"s, "steal"s,  "guest"s, "guest_nice"s};

std::unordered_map<string, long long> cpu_old = {
    {"totals", 0},  {"idles", 0}, {"user", 0},   {"nice", 0},
    {"system", 0},  {"idle", 0},  {"iowait", 0}, {"irq", 0},
...
string get_cpuName() {
  ...
  if (cpuinfo.good()) {
    for (string instr;
         getline(cpuinfo, instr, ':') and not instr.starts_with("model name");)
    ...
    if (cpuinfo.bad())
      ...
      if (not name.empty()) {
        ...
        if (name_vec.size() < 2)
          ...
          return capitalize(name_vec.at(1)) +
...
bool get_sensors() {
  ...
  try {
    ...
    if (fs::exists(fs::path("/sys/class/hwmon")) and
        access("/sys/class/hwmon", R_OK) != -1) {
      for (const auto &dir :
           fs::directory_iterator(fs::path("/sys/class/hwmon"))) {
        fs::path add_path = fs::canonical(dir.path());
        if (v_contains(search_paths, add_path) or
            v_contains(search_paths, add_path / "device"))
          continue;
        ...
        for (const auto &file : fs::directory_iterator(add_path)) {
          if (file.path().filename() == "device") {
            for (const auto &dev_file : fs::directory_iterator(file.path())) {
              ...
              if (dev_filename.starts_with("temp") and
                  dev_filename.ends_with("_input")) {
    ...
    if (not got_coretemp and
        fs::exists(fs::path("/sys/devices/platform/coretemp.0/hwmon"))) {
      for (auto &d : fs::directory_iterator(
               fs::path("/sys/devices/platform/coretemp.0/hwmon"))) {
        fs::path add_path = fs::canonical(d.path());

        for (const auto &file : fs::directory_iterator(add_path)) {
          string filename = file.path().filename();
          if (filename.starts_with("temp") and filename.ends_with("_input") and
              not v_contains(search_paths, add_path)) {
            search_paths.push_back(add_path);
            got_coretemp = true;
            break;
          }
    ...
    if (not search_paths.empty()) {
      for (const auto &path : search_paths) {
        ...
        for (const auto &file : fs::directory_iterator(path)) {
          ...
          const int file_id =
              atoi(file.path().filename().c_str() + 4); // skip "temp" prefix
          ...
          const string basepath = file_path.erase(file_path.find(file_suffix),
                                                  file_suffix.length());
          const string label = readfile(fs::path(basepath + "label"),
                                        "temp" + to_string(file_id));
          const string sensor_name = pname + "/" + label;
          const int64_t temp =
              stol(readfile(fs::path(basepath + "input"), "0")) / 1000;
          const int64_t crit =
              stol(readfile(fs::path(basepath + "crit"), "95000")) / 1000;

          found_sensors[sensor_name] =
              Sensor{fs::path(basepath + "input"), temp, crit};

          if (not got_cpu and
              (label.starts_with("Package id") or label.starts_with("Tdie") or
               label.starts_with("SoC Temperature"))) {
            got_cpu = true;
            cpu_sensor = sensor_name;
          } else if (label.starts_with("Core") or label.starts_with("Tccd")) {
            got_coretemp = true;
            if (not v_contains(core_sensors, sensor_name))
    ...
    if (not got_cpu and fs::exists(fs::path("/sys/class/thermal"))) {
      ...
      for (int i = 0; fs::exists(fs::path(rootpath + to_string(i))); i++) {
        ...
        if (not fs::exists(basepath / "temp"))
          continue;
        ...
        int64_t crit = 0;
        for (int ii = 0;
             fs::exists(basepath / fmt::format("trip_point_{}_temp", ii));
             ii++) {
          const string trip_type =
              readfile(basepath / fmt::format("trip_point_{}_type", ii));
          if (not is_in(trip_type, "high", "critical"))
            continue;
          auto &val = (trip_type == "high" ? high : crit);
          val = stol(readfile(basepath / fmt::format("trip_point_{}_temp", ii),
                              "0")) /
                1000;
        }
        if (high < 1)
  ...
  if (cpu_sensor.empty() and not found_sensors.empty()) {
    for (const auto &[name, sensor] : found_sensors) {
      if (str_to_lower(name).contains("cpu") or
          str_to_lower(name).contains("k10temp")) {
        cpu_sensor = name;
        break;
      }
    }
    if (cpu_sensor.empty()) {
      ...
      Logger::warning("No good candidate for cpu sensor found, using random "
                      "from all found sensors.");
    }
  }
...
static void update_sensors() {
  ...
  const auto &cpu_sensor =
      (not Config::getS("cpu_sensor").empty() and
               found_sensors.contains(Config::getS("cpu_sensor"))
           ? Config::getS("cpu_sensor")
           : Cpu::cpu_sensor);

  found_sensors.at(cpu_sensor).temp =
      stol(readfile(found_sensors.at(cpu_sensor).path, "0")) / 1000;
  ...
  if (Config::getB("show_coretemp") and not cpu_temp_only) {
    for (vector<string_view> done; const auto &sensor : core_sensors) {
      if (v_contains(done, sensor))
        continue;
      found_sensors.at(sensor).temp =
          stol(readfile(found_sensors.at(sensor).path, "0")) / 1000;
      done.push_back(sensor);
    }
    for (const auto &[core, temp] : core_mapping) {
      if (cmp_less(core + 1, current_cpu.temp.size()) and
          cmp_less(temp, core_sensors.size())) {
        current_cpu.temp.at(core + 1).push_back(
            found_sensors.at(core_sensors.at(temp)).temp);
  ...
  try {
    ...
    if (not frequencies.empty()) {
      ...
      if (freq_mode == "average") {
        hz = std::accumulate(frequencies.begin(), frequencies.end(), 0.0) /
    ...
    if (hz <= 0.0) {
      ...
      if (cpufreq.good()) {
        while (cpufreq.ignore(SSmax, '\n')) {
          // peek is caps sensitive so it was skipping 'CPU MHz'. This aims to
          // fix it.
    ...
    if (hz <= 1 or hz >= 999999999)
      throw std::runtime_error(
          "Failed to read /sys/devices/system/cpu/cpufreq/policy and "
          "/proc/cpuinfo.");
...
auto get_core_mapping() -> std::unordered_map<int, int> {
  ...
  //? If core mapping from cpuinfo was incomplete try to guess remainder, if
  //missing completely, map 0-0 1-1 2-2 etc.
  if (cmp_less(core_map.size(), Shared::coreCount)) {
    if (Shared::coreCount % 2 == 0 and
        (long) core_map.size() == Shared::coreCount / 2) {
  ...
  if (not custom_map.empty()) {
    try {
      for (const auto &split : ssplit(custom_map)) {
        ...
        if (not core_map.contains(change_id) or
            cmp_greater(new_id, core_sensors.size()))
          continue;
...
struct battery {
  fs::path base_dir, energy_now, charge_now, energy_full, charge_full,
      power_now, current_now, voltage_now, status, online;
  string device_type;
...
auto get_battery() -> tuple<int, float, long, string> {
    ...
    try {
      if (fs::exists("/sys/class/power_supply")) {
        for (const auto &d :
          ...
          fs::path bat_dir;
          try {
            if (not d.is_directory() or not fs::exists(d.path() / "type") or
                not fs::exists(d.path() / "present") or
          ...
          if (not new_bat.use_energy_or_charge and
          ...
          } else if ((fs::exists(bat_dir / "current_now")) and
                     (fs::exists(bat_dir / "voltage_now"))) {
  ...
  if (auto_sel.empty()) {
    ...
    if (auto_sel.empty())
      auto_sel = batteries.begin()->first;
  }

  auto &b = (battery_sel != "Auto" and batteries.contains(battery_sel)
                 ? batteries.at(battery_sel)
                 : batteries.at(auto_sel));
  ...
  if (b.use_energy_or_charge and percent < 0) {
    try {
      percent = round(100.0 * stod(readfile(b.energy_now, "-1")) /
                      stod(readfile(b.energy_full, "1")));
    } catch (const std::invalid_argument &) {
    } catch (const std::out_of_range &) {
    }
  }
  if (b.use_energy_or_charge and percent < 0) {
    try {
      percent = round(100.0 * stod(readfile(b.charge_now, "-1")) /
                      stod(readfile(b.charge_full, "1")));
    } catch (const std::invalid_argument &) {
    } catch (const std::out_of_range &) {
    }
  }
  ...
  if (not is_in(status, "charging", "full")) {
    if (b.use_energy_or_charge) {
      if (not b.power_now.empty()) {
        try {
          seconds = abs(round(stod(readfile(b.energy_now, "0")) /
                              stod(readfile(b.power_now, "1")) * 3600));
        } catch (const std::invalid_argument &) {
        } catch (const std::out_of_range &) {
        }
      } else if (not b.current_now.empty()) {
        try {
          seconds = abs(round(stod(readfile(b.charge_now, "0")) /
                              stod(readfile(b.current_now, "1")) * 3600));
        } catch (const std::invalid_argument &) {
        } catch (const std::out_of_range &) {
        }
      }
  ...
  //? Or get seconds to full
    ...
    if (b.use_energy_or_charge) {
      if (not b.power_now.empty()) {
        try {
          seconds = (round(stod(readfile(b.energy_full, "0")) -
                           round(stod(readfile(b.energy_now, "0")))) /
        ...
        try {
          seconds = (round(stod(readfile(b.charge_full, "0")) -
                           stod(readfile(b.charge_now, "0"))) /
  ...
  if (b.use_power) {
    if (not b.power_now.empty()) {
      ...
      try {
        watts = stof(readfile(b.current_now, "-1")) / 1000000.0F *
                stof(readfile(b.voltage_now, "1")) / 1000000.0F;
      } catch (const std::invalid_argument &) {
      } catch (const std::out_of_range &) {
      }
    }
...
static constexpr auto detect_active_cpus() {
  ...
  if (buf.empty()) {
    return std::views::iota(0, Shared::coreCount) |
           std::ranges::to<std::vector<std::int32_t>>();
  }

  return buf | std::views::split(',') |
         std::views::transform([](auto &&range) -> auto {
...
auto collect(bool no_update) -> cpu_info & {
  ...
  try {
    ...
    for (; i <= target or (cread.good() and cread.peek() == 'c'); i++) {
      ...
      if ((not cread.good() or cread.peek() != 'c') and i <= target) {
        ...
        if (i == 0)
          ...
          if (cpuNum >= target - 1)
            target = cpuNum + (cread.peek() == 'c' ? 2 : 1);
          ...
          while (i - 1 < cpuNum) {
            ...
            if (cpu.core_percent.at(i - 1).size() > 40)
              cpu.core_percent.at(i - 1).pop_front();
            i++;
          }
        }

        //? Expected on kernel 2.6.3> : 0=user, 1=nice, 2=system, 3=idle,
        //4=iowait, 5=irq, 6=softirq, 7=steal, 8=guest, 9=guest_nice
        ...
        const long long totals =
            max(0ll, total_sum - (times.size() > 8
                                      ? std::accumulate(times.begin() + 8,
                                                        times.end(), 0ll)
                                      : 0));

        //? Add iowait field if present
        const long long idles =
            max(0ll, times.at(3) + (times.size() > 4 ? times.at(4) : 0));
        ...
        if (i == 0) {
          ...
          cpu.cpu_percent.at("total").push_back(
              clamp((long long)round((double)(calc_totals - calc_idles) * 100 /
                                     calc_totals),
                    0ll, 100ll));
          ...
          for (int ii = 0; const auto &val : times) {
            cpu.cpu_percent.at(time_names.at(ii))
                .push_back(
                    clamp((long long)round(
                              (double)(val - cpu_old.at(time_names.at(ii))) *
                              100 / calc_totals),
                          0ll, 100ll));
            cpu_old.at(time_names.at(ii)) = val;

            //? Reduce size if there are more values than needed for graph
            while (cmp_greater(cpu.cpu_percent.at(time_names.at(ii)).size(),
                               width * 2))
        ...
        //? Calculate cpu total for each core
          ...
          const long long calc_totals =
              max(1ll, totals - core_old_totals.at(i - 1));
          const long long calc_idles =
              max(0ll, idles - core_old_idles.at(i - 1));
          core_old_totals.at(i - 1) = totals;
          core_old_idles.at(i - 1) = idles;

          cpu.core_percent.at(i - 1).push_back(
              clamp((long long)round((double)(calc_totals - calc_idles) * 100 /
                                     calc_totals),
                    0ll, 100ll));
        }
    ...
    //? Notify main thread to redraw screen if we found more cores than
    //previously detected
    if (cmp_greater(cpu.core_percent.size(), Shared::coreCount)) {
      Logger::debug("Changing CPU max corecount from {} to {}.",
                    Shared::coreCount, cpu.core_percent.size());
      Runner::coreNum_reset = true;
      Shared::coreCount = cpu.core_percent.size();
      while (cmp_less(current_cpu.temp.size(), cpu.core_percent.size() + 1))
        current_cpu.temp.push_back({0});
    }
...
bool init() {
  ...
  if (!nvml_dl_handle) {
    Logger::info(
        "Failed to load libnvidia-ml.so, NVIDIA GPUs will not be detected: {}",
        dlerror());
    return false;
  }
...
#define LOAD_SYM(NAME)                                                         \
  if ((NAME = (decltype(NAME))load_nvml_sym(#NAME)) == nullptr)                \
  return false
...
#undef LOAD_SYM
  ...
  if (result != NVML_SUCCESS) {
    Logger::debug(
        "Failed to initialize NVML, NVIDIA GPUs will not be detected: {}",
  ...
  if (result != NVML_SUCCESS) {
    Logger::warning("NVML: Failed to get device count: {}",
                    nvmlErrorString(result));
    return false;
  }
...
bool shutdown() {
  ...
  if (NVML_SUCCESS == result) {
    ...
    Logger::warning("Failed to shutdown NVML: {}", nvmlErrorString(result));

  return !initialized;
}

template <bool is_init> // collect<1> is called in Nvml::init(), and populates
                        // gpus.supported_functions
                        bool
                        collect(gpu_info
                                    *gpus_slice) { // raw pointer to vector
                                                   // data, size == device_count
  if (!initialized)
    return false;
  ...
  for (unsigned int i = 0; i < device_count; ++i) {
    if constexpr (is_init) {
      ...
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get device handle: {}",
                        nvmlErrorString(result));
        gpus[i].supported_functions = {false, false, false, false, false,
      ...
      result =
          nvmlDeviceGetName(devices[i], name, NVML_DEVICE_NAME_BUFFER_SIZE);
      if (result != NVML_SUCCESS)
        Logger::warning("NVML: Failed to get device name: {}",
                        nvmlErrorString(result));
      else {
      ...
      if (result != NVML_SUCCESS)
        Logger::warning("NVML: Failed to get maximum GPU power draw, "
                        "defaulting to 225W: {}",
                        nvmlErrorString(result));
      else {
      ...
      result = nvmlDeviceGetTemperatureThreshold(
          devices[i], NVML_TEMPERATURE_THRESHOLD_SHUTDOWN, &temp_max);
      if (result != NVML_SUCCESS)
        Logger::warning("NVML: Failed to get maximum GPU temperature, "
                        "defaulting to 110°C: {}",
                        nvmlErrorString(result));
      else
    ...
    //? PCIe link speeds, the data collection takes >=20ms each call so they run
    //on separate threads
    if (gpus_slice[i].supported_functions.pcie_txrx and
        (Config::getB("nvml_measure_pcie_speeds") or is_init)) {
      pcie_tx_thread = std::thread([gpus_slice, i]() {
        unsigned int tx;
        nvmlReturn_t result = nvmlDeviceGetPcieThroughput(
            devices[i], NVML_PCIE_UTIL_TX_BYTES, &tx);
        if (result != NVML_SUCCESS) {
          Logger::warning("NVML: Failed to get PCIe TX throughput: {}",
                          nvmlErrorString(result));
          if constexpr (is_init)
            gpus_slice[i].supported_functions.pcie_txrx = false;
        } else
      ...
      pcie_rx_thread = std::thread([gpus_slice, i]() {
        unsigned int rx;
        nvmlReturn_t result = nvmlDeviceGetPcieThroughput(
            devices[i], NVML_PCIE_UTIL_RX_BYTES, &rx);
        if (result != NVML_SUCCESS) {
          Logger::warning("NVML: Failed to get PCIe RX throughput: {}",
                          nvmlErrorString(result));
        } else
    ...
    if (gpus_slice[i].supported_functions.gpu_utilization) {
      ...
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get GPU utilization: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
          gpus_slice[i].supported_functions.gpu_utilization = false;
        if constexpr (is_init)
          gpus_slice[i].supported_functions.mem_utilization = false;
      } else {
        gpus_slice[i]
            .gpu_percent.at("gpu-totals")
            .push_back((long long)utilization.gpu);
        gpus_slice[i].mem_utilization_percent.push_back(
            (long long)utilization.memory);
      }
    }
    ...
    if (gpus_slice[i].supported_functions.gpu_clock) {
      ...
      result =
          nvmlDeviceGetClockInfo(devices[i], NVML_CLOCK_GRAPHICS, &gpu_clock);
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get GPU clock speed: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
    ...
    if (gpus_slice[i].supported_functions.mem_clock) {
      ...
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get VRAM clock speed: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
    ...
    if (gpus_slice[i].supported_functions.pwr_usage) {
      ...
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get GPU power usage: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
        ...
        gpus_slice[i]
            .gpu_percent.at("gpu-pwr-totals")
            .push_back(
                clamp((long long)round((double)gpus_slice[i].pwr_usage * 100.0 /
                                       (double)gpus_slice[i].pwr_max_usage),
                      0ll, 100ll));
      }
    }
    ...
    if (gpus_slice[i].supported_functions.pwr_state) {
      ...
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get GPU power state: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
    ...
    if (gpus_slice[i].supported_functions.temp_info) {
      if (Config::getB("check_temp")) {
        unsigned int temp;
        nvmlReturn_t result =
            nvmlDeviceGetTemperature(devices[i], NVML_TEMPERATURE_GPU, &temp);
        if (result != NVML_SUCCESS) {
          Logger::warning("NVML: Failed to get GPU temperature: {}",
                          nvmlErrorString(result));
          if constexpr (is_init)
    ...
    if (gpus_slice[i].supported_functions.mem_total) {
      ...
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get VRAM info: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
        ...
        auto used_percent = (long long)round((double)memory.used * 100.0 /
                                             (double)memory.total);
    ...
    if (gpus_slice[i].supported_functions.encoder_utilization) {
      ...
      result = nvmlDeviceGetEncoderUtilization(devices[i], &utilization,
                                               &samplingPeriodUs);
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get encoder utilization: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
    ...
    if (gpus_slice[i].supported_functions.decoder_utilization) {
      unsigned int utilization;
      unsigned int samplingPeriodUs;
      result = nvmlDeviceGetDecoderUtilization(devices[i], &utilization,
                                               &samplingPeriodUs);
      if (result != NVML_SUCCESS) {
        Logger::warning("NVML: Failed to get decoder utilization: {}",
                        nvmlErrorString(result));
        if constexpr (is_init)
    ...
    result = nvmlDeviceGetComputeRunningProcesses_v3(device, &proc_info_len,
    proc_info); if (result != NVML_SUCCESS) { Logger::warning("NVML: Failed to
    get compute processes: {}", nvmlErrorString(result)); } else { for (unsigned
    int i = 0; i < proc_info_len; ++i)
                    gpus_slice[i].graphics_processes.push_back({proc_info[i].pid,
    proc_info[i].usedGpuMemory});
    }*/

    // nvTimer.stop_rename_reset("Nv pcie thread join");
    //? Join PCIE TX/RX threads
    if constexpr (is_init) { // there doesn't seem to be a better way to do
                             // this, but this should be fine considering it's
                             // just 2 lines
      pcie_tx_thread.join();
      pcie_rx_thread.join();
    } else if (gpus_slice[i].supported_functions.pcie_txrx and
...
bool init() {
  ...
  if (!rsmi_dl_handle) {
    Logger::info(
        "Failed to load librocm_smi64.so, AMD GPUs will not be detected: {}",
        dlerror());
    return false;
  }
...
#define LOAD_SYM(NAME)                                                         \
  if ((NAME = (decltype(NAME))load_rsmi_sym(#NAME)) == nullptr)                \
  return false
...
#undef LOAD_SYM
  ...
  if (result != RSMI_STATUS_SUCCESS) {
    Logger::debug(
        "Failed to initialize ROCm SMI, AMD GPUs will not be detected");
    return false;
  }
  ...
  if (result != RSMI_STATUS_SUCCESS) {
    ...
    if ((rsmi_dev_gpu_clk_freq_get_v5 = (decltype(rsmi_dev_gpu_clk_freq_get_v5))
             load_rsmi_sym("rsmi_dev_gpu_clk_freq_get")) == nullptr)
      return false;
    // In the release tarballs of rocm 6.0.0 and 6.0.2 the version queried with
    // rsmi_version_get is 7.0.0.0
  } else if (version.major == 6 || version.major == 7) {
    if ((rsmi_dev_gpu_clk_freq_get_v6 = (decltype(rsmi_dev_gpu_clk_freq_get_v6))
             load_rsmi_sym("rsmi_dev_gpu_clk_freq_get")) == nullptr)
      return false;
  } else {
    Logger::warning(
        "ROCm SMI: Dynamic loading only supported for version 5 and 6");
    return false;
  }
...
bool collect(
    gpu_info *gpus_slice) { // raw pointer to vector data, size == device_count,
  ...
  for (uint32_t i = 0; i < device_count; ++i) {
    if constexpr (is_init) {
      ...
      if (result != RSMI_STATUS_SUCCESS)
        Logger::warning("ROCm SMI: Failed to get maximum GPU power draw, "
                        "defaulting to 225W");
      else {
        gpus_slice[i].pwr_max_usage =
            (long long)(max_power / 1000); // RSMI reports power in microWatts
        gpu_pwr_total_max += gpus_slice[i].pwr_max_usage;
      }

      //? Get temp_max
      int64_t temp_max;
      result = rsmi_dev_temp_metric_get(i, RSMI_TEMP_TYPE_EDGE, RSMI_TEMP_MAX,
                                        &temp_max);
      if (result != RSMI_STATUS_SUCCESS)
        Logger::warning("ROCm SMI: Failed to get maximum GPU temperature, "
                        "defaulting to 110°C");
      else
        gpus_slice[i].temp_max = (long long)temp_max;
    ...
    if (gpus_slice[i].supported_functions.gpu_utilization) {
      ...
      if (result != RSMI_STATUS_SUCCESS) {
        ...
        gpus_slice[i]
            .gpu_percent.at("gpu-totals")
            .push_back((long long)utilization);
    }
    ...
    if (gpus_slice[i].supported_functions.gpu_clock) {
      if (version_major == 5) {
        ...
        result =
            rsmi_dev_gpu_clk_freq_get_v5(i, RSMI_CLK_TYPE_SYS, &frequencies);
        if (result != RSMI_STATUS_SUCCESS) {
          Logger::warning("ROCm SMI: Failed to get GPU clock speed: ");
          if constexpr (is_init)
            gpus_slice[i].supported_functions.gpu_clock = false;
        } else
          gpus_slice[i].gpu_clock_speed =
              (long long)frequencies.frequency[frequencies.current] /
              1000000; // Hz to MHz
      } else if (version_major == 6 || version_major == 7) {
        rsmi_frequencies_t_v6 frequencies;
        result =
            rsmi_dev_gpu_clk_freq_get_v6(i, RSMI_CLK_TYPE_SYS, &frequencies);
        if (result != RSMI_STATUS_SUCCESS) {
          Logger::warning("ROCm SMI: Failed to get GPU clock speed: ");
          if constexpr (is_init)
            gpus_slice[i].supported_functions.gpu_clock = false;
    ...
    if (gpus_slice[i].supported_functions.mem_clock) {
      if (version_major == 5) {
        rsmi_frequencies_t_v5 frequencies;
        result =
            rsmi_dev_gpu_clk_freq_get_v5(i, RSMI_CLK_TYPE_MEM, &frequencies);
        if (result != RSMI_STATUS_SUCCESS) {
          Logger::warning("ROCm SMI: Failed to get VRAM clock speed: ");
          if constexpr (is_init)
            gpus_slice[i].supported_functions.mem_clock = false;
        } else
          gpus_slice[i].mem_clock_speed =
              (long long)frequencies.frequency[frequencies.current] /
              1000000; // Hz to MHz
      } else if (version_major == 6 || version_major == 7) {
        rsmi_frequencies_t_v6 frequencies;
        result =
            rsmi_dev_gpu_clk_freq_get_v6(i, RSMI_CLK_TYPE_MEM, &frequencies);
        if (result != RSMI_STATUS_SUCCESS) {
          Logger::warning("ROCm SMI: Failed to get VRAM clock speed: ");
          if constexpr (is_init)
            gpus_slice[i].supported_functions.mem_clock = false;
    ...
    if (gpus_slice[i].supported_functions.temp_info) {
      if (Config::getB("check_temp") or is_init) {
        int64_t temp;
        result = rsmi_dev_temp_metric_get(i, RSMI_TEMP_TYPE_EDGE,
                                          RSMI_TEMP_CURRENT, &temp);
        if (result != RSMI_STATUS_SUCCESS) {
    ...
    if (gpus_slice[i].supported_functions.mem_used) {
      ...
      if (result != RSMI_STATUS_SUCCESS) {
        ...
        if (gpus_slice[i].supported_functions.mem_total)
          gpus_slice[i]
              .gpu_percent.at("gpu-vram-totals")
              .push_back((long long)round((double)used * 100.0 /
                                          (double)gpus_slice[i].mem_total));
      }
    }

    //? PCIe link speeds
    if (gpus_slice[i].supported_functions.pcie_txrx and
        Config::getB("rsmi_measure_pcie_speeds")) {
      uint64_t tx, rx;
...
static uint64_t read_event_config(const string &pmu_device,
...
static uint64_t build_config(uint64_t event_id, uint16_t gt_id,
                             uint16_t engine_class, uint16_t engine_instance) {
  uint64_t config = 0;
...
static bool enumerate_engines(int drm_fd, vector<XeEngine> &engines) {
  ...
  for (uint32_t i = 0; i < result->num_engines; ++i) {
    ...
    if (inst.engine_class == DRM_XE_ENGINE_CLASS_VM_BIND)
      continue;
    engines.push_back(
        {inst.gt_id, inst.engine_class, inst.engine_instance, -1, -1, 0, 0});
  }
...
bool init(const string &card_path, const string &pmu_dev) {
  ...
  for (auto &eng : state.engines) {
    uint64_t active_cfg = build_config(active_event, eng.gt_id,
                                       eng.engine_class, eng.engine_instance);
    uint64_t total_cfg = build_config(total_event, eng.gt_id, eng.engine_class,
                                      eng.engine_instance);
    ...
    if (state.group_fd == -1) {
      ...
      eng.active_fd =
          open_perf_counter(state.pmu_type, active_cfg, state.group_fd);
    }
    eng.total_fd = open_perf_counter(state.pmu_type, total_cfg, state.group_fd);

    if (eng.active_fd >= 0)
      state.member_fds.push_back(eng.active_fd);
    if (eng.total_fd >= 0)
      state.member_fds.push_back(eng.total_fd);
  }
  ...
  if (freq_event != 0) {
    ...
    if (state.freq_fd >= 0)
      state.member_fds.push_back(state.freq_fd);
  }
...
template <bool is_init> bool collect(gpu_info *gpus_slice) {
  ...
  if constexpr (is_init) {
    gpus_slice->supported_functions = {.gpu_utilization =
                                           !state.engines.empty(),
  ...
  double max_util = 0;
  for (auto &eng : state.engines) {
    if (eng.active_fd < 0 || eng.total_fd < 0)
      continue;
    uint64_t active_val = 0, total_val = 0;
    if (read(eng.active_fd, &active_val, sizeof(active_val)) < 0)
      continue;
    if (read(eng.total_fd, &total_val, sizeof(total_val)) < 0)
      continue;
  ...
  gpus_slice->gpu_percent.at("gpu-totals")
      .push_back(clamp((long long)round(max_util), 0ll, 100ll));
...
bool init() {
  ...
  if (!gpu_path) {
    Logger::debug(
        "Failed to find Intel GPU sysfs path, Intel GPUs will not be detected");
    return false;
  }
  ...
  if (!gpu_device_id) {
    Logger::debug(
        "Failed to find Intel GPU device ID, Intel GPUs will not be detected");
    return false;
  }
  ...
  if (pmu_device.empty()) {
    Logger::debug("Intel GPU: Could not find PMU device for driver {}",
                  driver_name);
    return false;
  }

  if (driver_name == "xe") {
    if (!Xe::init(card_path, pmu_device)) {
      Logger::debug("Intel GPU: Xe initialization failed");
      return false;
    }
  } else {
    engines = discover_engines(pmu_device.c_str());
    if (!engines) {
      Logger::debug(
          "Failed to find Intel GPU engines, Intel GPUs will not be detected");
      return false;
    }
  ...
  gpu_names[Nvml::device_count + Rsmi::device_count] = device_name;
...
template <bool is_init> bool collect(gpu_info *gpus_slice) {
  ...
  if (driver_name == "xe") {
    return Xe::collect<is_init>(gpus_slice);
  }

  if constexpr (is_init) {
    gpus_slice->supported_functions = {.gpu_utilization = true,
                                       .mem_utilization = false,
                                       .gpu_clock = true,
                                       .mem_clock = false,
                                       .pwr_usage = true,
  ...
  gpus_slice->gpu_percent.at("gpu-totals")
      .push_back((long long)round(max_util));

  double pwr = pmu_calc(&engines->r_gpu.val, 1, t, engines->r_gpu.scale);
  ...
  gpus_slice->gpu_percent.at("gpu-pwr-totals")
      .push_back(clamp((long long)round((double)gpus_slice->pwr_usage * 100.0 /
                                        (double)gpus_slice->pwr_max_usage),
                       0ll, 100ll));

  double freq = pmu_calc(&engines->freq_act.val, 1, t, 1);
...
auto collect(bool no_update) -> vector<gpu_info> & {
  if (Runner::stopping or (no_update and not gpus.empty()))
    return gpus;
  ...
  //* Collect data
  Nvml::collect<0>(
      gpus.data()); // raw pointer to vector data, size == Nvml::device_count
  Rsmi::collect<0>(gpus.data() +
                   Nvml::device_count); // size = Rsmi::device_count
  Intel::collect<0>(gpus.data() + Nvml::device_count +
                    Rsmi::device_count); // size = Intel::device_count
  ...
  for (auto &gpu : gpus) {
    ...
    if (width != 0) {
      ...
      //? Memory usage
      while (
          cmp_greater(gpu.gpu_percent.at("gpu-vram-totals").size(), width / 2))
  ...
  if (mem_total != 0)
    shared_gpu_percent.at("gpu-vram-total")
        .push_back(mem_usage_total / mem_total);
  if (gpu_pwr_total_max != 0)
    shared_gpu_percent.at("gpu-pwr-total")
        .push_back(pwr_total / gpu_pwr_total_max);

  if (width != 0) {
    while (cmp_greater(shared_gpu_percent.at("gpu-average").size(), width * 2))
      shared_gpu_percent.at("gpu-average").pop_front();
    while (
        cmp_greater(shared_gpu_percent.at("gpu-pwr-total").size(), width * 2))
      shared_gpu_percent.at("gpu-pwr-total").pop_front();
    while (
        cmp_greater(shared_gpu_percent.at("gpu-vram-total").size(), width * 2))
...
static auto convert_ascii_escapes(const std::string &input) -> std::string {
  ...
  for (std::size_t i = 0; i < input.size(); ++i) {
    if (input[i] == '\\' &&
        ...
        i + 3 < input.size() && std::isdigit(input[i + 1]) &&
        std::isdigit(input[i + 2]) && std::isdigit(input[i + 3])) {
      ...
      //   + 0
      //   = 32 (ascii space)
      int value = ((input[i + 1] - '0') * 64) + ((input[i + 2] - '0') * 8) +
                  (input[i + 3] - '0');
...
fs::path get_zfs_stat_file(const string &device_name, size_t dataset_name_start,
                           bool zfs_hide_datasets);
...
uint64_t get_totalMem() {
  ...
  if (not meminfo.good() or totalMem == 0)
    throw std::runtime_error(
        "Could not get total memory size from /proc/meminfo");
...
auto collect(bool no_update) -> mem_info & {
  ...
  if (zfs_arc_cached) {
    ...
    if (arcstats.good()) {
      for (string label; arcstats >> label;) {
        if (label == "c_min") {
          arcstats >> arc_min_size >>
              arc_min_size; // double read skips type column
        } else if (label == "size") {
  ...
  if (meminfo.good()) {
    ...
    for (string label; meminfo.peek() != 'D' and meminfo >> label;) {
      if (label == "MemFree:") {
        ...
        if (not show_swap and not swap_disk)
          break;
    ...
    if (zfs_arc_cached) {
      ...
      // The ARC will not shrink below arc_min_size, so that memory is not
      // available
      if (arc_size > arc_min_size)
        mem.stats.at("available") += arc_size - arc_min_size;
    }
    mem.stats.at("used") = totalMem - (mem.stats.at("available") <= totalMem
  ...
  for (const auto &name : mem_names) {
    mem.percent.at(name).push_back(
        round((double)mem.stats.at(name) * 100 / totalMem));
  ...
  if (show_disks) {
    ...
    try {
      ...
      static std::unordered_map<string, future<pair<disk_info, int>>>
          disks_stats_promises;
      ifstream diskread;
        ...
        if (diskread.good()) {
          for (string instr; diskread >> instr;) {
            if (not instr.starts_with('#')) {
              ...
              if (instr == "/")
                fstab.push_back("/mnt");
              else if (not is_in(instr, "none", "swap"))
                fstab.push_back(instr);
#else
              if (not is_in(instr, "none", "swap"))
                fstab.push_back(instr);
#endif
            }
            diskread.ignore(SSmax, '\n');
      ...
      if (diskread.good()) {
        ...
        while (not diskread.eof()) {
          ...
          // A mountpoint can ascii escape codes, which will not work with
          // `statvfs`.
          mountpoint = convert_ascii_escapes(mountpoint);

          if (v_contains(ignore_list, mountpoint) or
              v_contains(found, mountpoint))
            continue;
          ...
          if ((not use_fstab and not only_physical) or
              (use_fstab and v_contains(fstab, mountpoint)) or
              (not use_fstab and only_physical and
               v_contains(fstypes, fstype))) {
            found.push_back(mountpoint);
            if (not v_contains(last_found, mountpoint))
              redraw = true;

            //? Save mountpoint, name, fstype, dev path and path to /sys/block
            //stat file
            if (not disks.contains(mountpoint)) {
              disks[mountpoint] =
                  disk_info{fs::canonical(dev, ec),
                            fs::path(mountpoint).filename(), fstype};
              if (disks.at(mountpoint).dev.empty())
                disks.at(mountpoint).dev = dev;
#ifdef SNAPPED
              if (mountpoint == "/mnt")
                disks.at(mountpoint).name = "root";
#endif
              if (disks.at(mountpoint).name.empty())
                disks.at(mountpoint).name =
              ...
              while (devname.size() >= 2) {
                ...
                if (fs::exists(stat, ec) and access(stat.c_str(), R_OK) == 0) {
                  const auto mount_stat =
                      fmt::format("/sys/block/{}/{}/stat", devname,
                  ...
                  disks.at(mountpoint).stat = get_zfs_stat_file(
                      dev, zfs_dataset_name_start, zfs_hide_datasets);
                  if (disks.at(mountpoint).stat.empty()) {
                    Logger::debug("Failed to get ZFS stat file for device {}",
                                  dev);
                  }
                  break;
                }
        ...
        throw std::runtime_error(
            "Failed to get mounts from /etc/mtab and /proc/self/mounts");
      diskread.close();
      ...
      for (auto it = disks.begin(); it != disks.end();) {
        ...
        if (auto promises_it = disks_stats_promises.find(mountpoint);
            promises_it != disks_stats_promises.end()) {
          ...
          if (promise_res.second != -1) {
            ...
            Logger::warning("Failed to get disk/partition stats for mount "
                            "\"{}\" with statvfs error code: {}. Ignoring...",
                            mountpoint, promise_res.second);
            it = disks.erase(it);
            continue;
          }
        ...
        disks_stats_promises[mountpoint] =
            async(std::launch::async,
                  [mountpoint, free_priv]() -> pair<disk_info, int> {
                    struct statvfs vfs;
                    disk_info disk;
                    ...
                    disk.free =
                        (free_priv ? vfs.f_bfree : vfs.f_bavail) * vfs.f_frsize;
      ...
      for (auto &[ignored, disk] : disks) {
        if (disk.stat.empty() or access(disk.stat.c_str(), R_OK) != 0)
          continue;
        if (disk.fstype == "zfs" && zfs_hide_datasets &&
            zfs_collect_pool_total_stats(disk)) {
          disk_ios++;
          continue;
        }
        ...
        if (diskread.good()) {
          ...
          if (disk.fstype == "zfs") {
            ...
            for (int i = 0; i < 3; i++)
              diskread.ignore(numeric_limits<streamsize>::max(), '\n');
            // skip characters until '4' is reached, indicating data type 4,
            // next value will be out target
            ...
            if (disk.io_write.empty())
              ...
              disk.io_write.push_back(
                  max((int64_t)0, (sectors_write - disk.old_io.at(1))));
            ...
            if (disk.io_read.empty())
              ...
              disk.io_read.push_back(
                  max((int64_t)0, (sectors_read - disk.old_io.at(0))));
            ...
            if (disk.io_activity.empty())
              ...
              disk.io_activity.push_back(
                  max((int64_t)0, (io_ticks - disk.old_io.at(2))));
            ...
            if (disk.io_read.empty())
              ...
              disk.io_read.push_back(
                  max((int64_t)0, (sectors_read - disk.old_io.at(0)) * 512));
            disk.old_io.at(0) = sectors_read;
            while (cmp_greater(disk.io_read.size(), width * 2))
              disk.io_read.pop_front();
            ...
            if (disk.io_write.empty())
              ...
              disk.io_write.push_back(
                  max((int64_t)0, (sectors_write - disk.old_io.at(1)) * 512));
            disk.old_io.at(1) = sectors_write;
            while (cmp_greater(disk.io_write.size(), width * 2))
              disk.io_write.pop_front();
            ...
            if (uptime == old_uptime || disk.io_activity.empty())
              ...
              disk.io_activity.push_back(
                  clamp((long)round((double)(io_ticks - disk.old_io.at(2)) /
                                    (uptime - old_uptime) / 10),
                        0l, 100l));
            disk.old_io.at(2) = io_ticks;
            while (cmp_greater(disk.io_activity.size(), width * 2))
              disk.io_activity.pop_front();
...
fs::path get_zfs_stat_file(const string &device_name, size_t dataset_name_start,
  ...
  if (dataset_name_start != std::string::npos) { // device is a dataset
    zfs_pool_stat_path = Shared::procPath / "spl/kstat/zfs" /
                         device_name.substr(0, dataset_name_start);
  } else { // device is a pool
    zfs_pool_stat_path = Shared::procPath / "spl/kstat/zfs" / device_name;
  }

  // looking through all files that start with 'objset' to find the one
    ...
    for (const auto &file : fs::directory_iterator(zfs_pool_stat_path)) {
      ...
      if (filename.starts_with("objset")) {
        ...
        if (filestream.good()) {
          ...
          // skip characters until '7' is reached, indicating data type 7, next
          // value will be object name
...
bool zfs_collect_pool_total_stats(struct disk_info &disk) {
    ...
    if ((file.path().filename()).string().starts_with("objset")) {
      ...
      if (diskread.good()) {
        try {
          ...
          for (int i = 0; i < 3; i++)
            diskread.ignore(numeric_limits<streamsize>::max(), '\n');
          // skip characters until '4' is reached, indicating data type 4, next
          // value will be out target
          diskread.ignore(numeric_limits<streamsize>::max(), '4');
          diskread >> io_ticks;
  ...
  if (disk.io_write.empty())
    ...
    disk.io_write.push_back(
        max((int64_t)0, (bytes_write_total - disk.old_io.at(1))));
  ...
  if (disk.io_read.empty())
    ...
    disk.io_read.push_back(
        max((int64_t)0, (bytes_read_total - disk.old_io.at(0))));
  ...
  if (disk.io_activity.empty())
    ...
    disk.io_activity.push_back(
        max((int64_t)0, (io_ticks_total - disk.old_io.at(2))));
...
int errors{};
std::unordered_map<string, uint64_t> graph_max = {{"download", {}},
                                                  {"upload", {}}};
std::unordered_map<string, array<int, 2>> max_count = {{"download", {}},
                                                       {"upload", {}}};
bool rescale{true};
uint64_t timestamp{};
...
auto collect(bool no_update) -> net_info & {
  ...
  if (not no_update and errors < 3) {
    ...
    int family = 0;
    static_assert(INET6_ADDRSTRLEN >=
                  INET_ADDRSTRLEN); // 46 >= 16, compile-time assurance.
    enum {
      IPBUFFER_MAXSIZE = INET6_ADDRSTRLEN
    }; // manually using the known biggest value, guarded by the above
       // static_assert
    ...
    for (auto *ifa = if_addrs.get(); ifa != nullptr; ifa = ifa->ifa_next) {
      ...
      if (not v_contains(interfaces, iface)) {
        ...
        // An interface can have more than one IP of the same family associated
        // with it, but we pick only the first one to show in the NET box. Note:
        // Interfaces without any IPv4 and IPv6 set are still valid and
        // monitorable!
      ...
      if (family == AF_INET) {
        if (net[iface].ipv4.empty()) {
          if (nullptr !=
            ...
            int errsv = errno;
            Logger::error("Net::collect() -> Failed to convert IPv4 to string "
                          "for iface {}, errno: {}",
                          iface, strerror(errsv));
          }
        }
      }
      //? Get IPv6 address
        ...
        if (net[iface].ipv6.empty()) {
          if (nullptr !=
            ...
            int errsv = errno;
            Logger::error("Net::collect() -> Failed to convert IPv6 to string "
                          "for iface {}, errno: {}",
                          iface, strerror(errsv));
          }
        }
      } // else, ignoring family==AF_PACKET (see man 3 getifaddrs) which is the
        // first one in the `for` loop.
    }

    //? Get total received and transmitted bytes + device address if no ip was
    //found
    for (const auto &iface : interfaces) {
      ...
      for (const string dir : {"download", "upload"}) {
        const fs::path sys_file = "/sys/class/net/" + iface + "/statistics/" +
                                  (dir == "download" ? "rx_bytes" : "tx_bytes");
        ...
        if (cmp_greater((unsigned long long)saved_stat.rollover +
                            (unsigned long long)val,
                        numeric_limits<uint64_t>::max())) {
          saved_stat.rollover = 0;
          saved_stat.last = 0;
        }
        saved_stat.speed = round((double)(val - saved_stat.last) /
                                 ((double)(new_timestamp - timestamp) / 1000));
        ...
        if (net_auto and selected_iface == iface) {
          if (net_sync and
              saved_stat.speed <
                  netif.stat.at(dir == "download" ? "upload" : "download")
                      .speed)
            continue;
    ...
    for (const auto &dir : {"download", "upload"}) {
      for (const auto &sel : {0, 1}) {
        if (rescale or max_count[dir][sel] >= 5) {
          ...
          graph_max[dir] = max(uint64_t(avg_speed * (sel == 0 ? 1.3 : 3.0)),
                               (uint64_t)10 << 10);
...
static void _collect_details(const size_t pid, const uint64_t uptime,
  ...
  if (detailed.entry.state != 'X')
    detailed.elapsed =
        sec_to_dhms(uptime - (detailed.entry.cpu_s / Shared::clkTck));
  else
  ...
  detailed.status = (proc_states.contains(detailed.entry.state))
                        ? proc_states.at(detailed.entry.state)
                        : "Unknown";
...
auto collect(bool no_update) -> vector<proc_info> & {
  ...
  bool sorted_change =
      (sorting != current_sort or reverse != current_rev or should_filter);
  ...
  //start----------------------------------------------
    ...
    if (should_filter_kernel and ++proc_clear_count >= 256) {
      ...
      //? In that event processes that acquire old kernel pids would also be
      //filtered out so we need to manually clean the cache every now and then.
      kernels_procs.clear();
    ...
    for (const auto &d : fs::directory_iterator(Shared::procPath)) {
      ...
      if (no_cache) {
        ...
        pread.close();
        //? Check for whitespace characters in name and set offset to get
        //correct fields from stat file
      ...
      try {
        for (;;) {
          ...
          case 22: //? Get cpu seconds if missing
            ...
            next_x = 24;
            continue;
          case 24: //? RSS memory (can be inaccurate, but parsing smaps
                   //increases total cpu usage by ~20x)
      ...
      //? Get RSS memory from /proc/[pid]/statm if value from /proc/[pid]/stat
      //looks wrong
      ...
      new_proc.cpu_p = clamp(round(cmult * 1000 * (cpu_t - new_proc.cpu_t) /
                                   max((uint64_t)1, cputimes - old_cputimes)) /
                                 10.0,
                             0.0, 100.0 * Shared::coreCount);

      //? Process cumulative cpu usage since process start
      new_proc.cpu_c =
          (double)cpu_t / max(1.0, (uptime * Shared::clkTck) - new_proc.cpu_s);
    ...
    //? Clear dead processes from current_procs and remove kernel processes if
    ...
    //? Set correct state of dead processes if paused
      ...
      for (auto &r : current_procs) {
        if (rng::find(found, r.pid) == found.end()) {
          if (r.state != 'X')
            r.death_time = round(uptime) - (r.cpu_s / Shared::clkTck);
          r.state = 'X';
  ...
  //* Sort processes
  if ((sorted_change or tree_mode_change) or
  ...
  if (tree and (not no_update or should_filter or sorted_change)) {
    ...
    if (toggle_children != -1) {
      auto collapser =
          rng::find(current_procs, toggle_children, &proc_info::pid);
    ...
    //? Stable sort to retain selected sorting among processes with the same
    //parent
    rng::stable_sort(current_procs, rng::less{}, &proc_info::ppid);

    //? Start recursive iteration over processes with the lowest shared parent
    //pids
    for (auto &p : rng::equal_range(current_procs, current_procs.at(0).ppid,
                                    rng::less{}, &proc_info::ppid)) {
      _tree_gen(p, current_procs, tree_procs, 0, false, filter, false,
                no_update, should_filter);
    }

    //? Recursive sort over tree structure to account for collapsed processes in
    //the tree
    int index = 0;
    tree_sort(tree_procs, sorting, reverse,
              (pause_proc_list and not(sorted_change or tree_mode_change)),
              index, current_procs.size());
    ...
    //? Move current selection/view to the selected process when
    //collapsing/expanding in the tree
    if (locate_selection) {
      int loc = rng::find(current_procs, Proc::selected_pid, &proc_info::pid)
                    ->tree_index;
      ...
      Config::ints.at("proc_selected") =
          loc - Config::ints.at("proc_start") + 1;
    }
  }
...
double system_uptime() {
  ...
  throw std::runtime_error(
      fmt::format("Failed to get uptime from {}", Shared::procPath / "uptime"));
}
} // namespace Tools
