	...
	auto collect(bool no_update) -> cpu_info& {
		...
		try {
			...
			if (cmp_greater(cpu.core_percent.size(), Shared::coreCount)) {
				Logger::debug(
					"Changing CPU max corecount from " + to_string(Shared::coreCount) + " to " +
					to_string(cpu.core_percent.size()) + "."
				);
			...
			if (cread.bad())
				...
				throw std::runtime_error("Cpu::collect() : " + string {e.what()});
		}
...
namespace Gpu {
	...
	#undef LOAD_SYM
			...
			if (result != NVML_SUCCESS) {
				Logger::debug(
					std::string("Failed to initialize NVML, NVIDIA GPUs will not be detected: ") +
		...
		bool collect(gpu_info* gpus_slice) { // raw pointer to vector data, size == device_count
			...
			for (unsigned int i = 0; i < device_count; ++i) {
				if constexpr (is_init) {
					...
					if (result != NVML_SUCCESS)
						Logger::warning(
							std::string("NVML: Failed to get maximum GPU power draw, defaulting to 225W: ") +
							nvmlErrorString(result)
						);
					else {
					...
					if (result != NVML_SUCCESS)
						Logger::warning(
							std::string("NVML: Failed to get maximum GPU temperature, defaulting to 110°C: ") +
							nvmlErrorString(result)
						);
					else
				...
				if (gpus_slice[i].supported_functions.pcie_txrx and
					...
					pcie_tx_thread = std::thread([gpus_slice, i]() {
						...
						if (result != NVML_SUCCESS) {
							Logger::warning(
								std::string("NVML: Failed to get PCIe TX throughput: ") + nvmlErrorString(result)
							);
					...
					pcie_rx_thread = std::thread([gpus_slice, i]() {
						...
						if (result != NVML_SUCCESS) {
							Logger::warning(
								std::string("NVML: Failed to get PCIe RX throughput: ") + nvmlErrorString(result)
							);
						} else
				...
				if (gpus_slice[i].supported_functions.mem_clock) {
					...
					if (result != NVML_SUCCESS) {
						Logger::warning(
							std::string("NVML: Failed to get VRAM clock speed: ") + nvmlErrorString(result)
						);
				...
				if (gpus_slice[i].supported_functions.temp_info) {
					if (Config::getB("check_temp")) {
						...
						if (result != NVML_SUCCESS) {
							Logger::warning(
								std::string("NVML: Failed to get GPU temperature: ") + nvmlErrorString(result)
							);
				...
				if (gpus_slice[i].supported_functions.encoder_utilization) {
					...
					if (result != NVML_SUCCESS) {
						Logger::warning(
							std::string("NVML: Failed to get encoder utilization: ") + nvmlErrorString(result)
						);
				...
				if (gpus_slice[i].supported_functions.decoder_utilization) {
					...
					if (result != NVML_SUCCESS) {
						Logger::warning(
							std::string("NVML: Failed to get decoder utilization: ") + nvmlErrorString(result)
						);
				...
				for (auto it = disks.begin(); it != disks.end();) {
					...
					if (auto promises_it = disks_stats_promises.find(mountpoint);
						...
						if (promise_res.second != -1) {
							...
							Logger::warning(
								"Failed to get disk/partition stats for mount \"" + mountpoint +
								"\" with statvfs error code: " + to_string(promise_res.second) + ". Ignoring..."
							);
							it = disks.erase(it);
							continue;
						}
				...
				if (family == AF_INET) {
					if (net[iface].ipv4.empty()) {
						if (nullptr != inet_ntop(
							...
							int errsv = errno;
							Logger::error(
								"Net::collect() -> Failed to convert IPv4 to string for iface " + string(iface) +
				...
				//? Get IPv6 address
					...
					if (net[iface].ipv6.empty()) {
						if (nullptr != inet_ntop(
							...
							int errsv = errno;
							Logger::error(
								"Net::collect() -> Failed to convert IPv6 to string for iface " + string(iface) +
								", errno: " + strerror(errsv)
							);
						}
					}
	...
	auto collect(bool no_update) -> vector<proc_info>& {
		...
		//* ---------------------------------------------Collection start----------------------------------------------
			...
			if (should_filter_kernel and ++proc_clear_count >= 256) {
				...
				// manually clean the cache every now and then.
				kernels_procs.clear();
					...
					for (;;) {
						...
						switch (x - offset) {
							...
							case 22: //? Get cpu seconds if missing
								...
								next_x = 24;
								continue;
							case 24: //? RSS memory (can be inaccurate, but parsing smaps increases total cpu usage by
									 //~20x)
