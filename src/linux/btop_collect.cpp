...
using std::clamp, std::string_literals::operator""s, std::cmp_equal,
    std::cmp_less, std::cmp_greater;
using std::ifstream, std::numeric_limits, std::streamsize, std::round, std::max,
    std::min;
...
void init() {
    ...
    if (coreCount < 1) {
        coreCount = 1;
        Logger::warning(
            "Could not determine number of cores, defaulting to 1.");
    }
    ...
    if (pageSize <= 0) {
        pageSize = 4096;
        Logger::warning(
            "Could not get system page size. Defaulting to 4096, processes "
            "memory usage might be incorrect.");
    }
    ...
    if (clkTck <= 0) {
        clkTck = 100;
        Logger::warning(
            "Could not get system clock ticks per second. Defaulting to 100, "
            "processes cpu usage might be incorrect.");
    }

    //? Init for namespace Cpu
    if (not fs::exists(Cpu::freq_path) or
        access(Cpu::freq_path.c_str(), R_OK) == -1)
        Cpu::freq_path.clear();
    ...
    Cpu::core_old_totals.insert(Cpu::core_old_totals.begin(), Shared::coreCount,
                                0);
    Cpu::core_old_idles.insert(Cpu::core_old_idles.begin(), Shared::coreCount,
                               0);
    Cpu::collect();
...
string get_cpuName() {
    ...
    if (cpuinfo.good()) {
        for (string instr; getline(cpuinfo, instr, ':') and
                           not instr.starts_with("model name");)
        ...
        if ((s_contains(name, "Xeon"s) or v_contains(name_vec, "Duo"s)) and
            v_contains(name_vec, "CPU"s)) {
            auto cpu_pos = v_index(name_vec, "CPU"s);
            if (cpu_pos < name_vec.size() - 1 and
                not name_vec.at(cpu_pos + 1).ends_with(')'))
            ...
            name =
                "Ryzen" +
                (ryz_pos < name_vec.size() - 1 ? ' ' + name_vec.at(ryz_pos + 1)
                                               : "") +
                (ryz_pos < name_vec.size() - 2 ? ' ' + name_vec.at(ryz_pos + 2)
                                               : "");
        } else if (s_contains(name, "Intel"s) and
                   v_contains(name_vec, "CPU"s)) {
            auto cpu_pos = v_index(name_vec, "CPU"s);
            if (cpu_pos < name_vec.size() - 1 and
                not name_vec.at(cpu_pos + 1).ends_with(')') and
        ...
        if (name.empty() and not name_vec.empty()) {
            ...
            for (const auto& replace :
                 {"Processor", "CPU", "(R)", "(TM)", "Intel", "AMD", "Core"}) {
...
bool get_sensors() {
    ...
    try {
        ...
        if (fs::exists(fs::path("/sys/class/hwmon")) and
            ...
            for (const auto& dir :
                ...
                if (s_contains(add_path, "coretemp")) got_coretemp = true;

                for (const auto& file : fs::directory_iterator(add_path)) {
                    if (string(file.path().filename()) == "device") {
                        for (const auto& dev_file :
                             fs::directory_iterator(file.path())) {
        ...
        if (not search_paths.empty()) {
            for (const auto& path : search_paths) {
                ...
                for (const auto& file : fs::directory_iterator(path)) {
                    ...
                    const int file_id = atoi(file.path().filename().c_str() +
                                             4);  // skip "temp" prefix
                    ...
                    const string basepath = file_path.erase(
                        file_path.find(file_suffix), file_suffix.length());
                    const string label = readfile(fs::path(basepath + "label"),
                                                  "temp" + to_string(file_id));
                    const string sensor_name = pname + "/" + label;
                    const int64_t temp =
                        stol(readfile(fs::path(basepath + "input"), "0")) /
                        1000;
                    const int64_t high =
                        stol(readfile(fs::path(basepath + "max"), "80000")) /
                        1000;
                    const int64_t crit =
                        stol(readfile(fs::path(basepath + "crit"), "95000")) /
                        1000;

                    found_sensors[sensor_name] = {fs::path(basepath + "input"),
                                                  label, temp, high, crit};

                    if (not got_cpu and (label.starts_with("Package id") or
                                         label.starts_with("Tdie"))) {
                        got_cpu = true;
                        cpu_sensor = sensor_name;
                    } else if (label.starts_with("Core") or
        ...
        if (not got_cpu and fs::exists(fs::path("/sys/class/thermal"))) {
            ...
            for (int i = 0; fs::exists(fs::path(rootpath + to_string(i)));
                ...
                found_sensors[sensor_name] = {basepath / "temp", label, temp,
                                              high, crit};
            }
        }
    ...
    if (cpu_sensor.empty() and not found_sensors.empty()) {
        ...
        if (cpu_sensor.empty()) {
            ...
            Logger::warning(
                "No good candidate for cpu sensor found, using random from all "
                "found sensors.");
        }
    }
...
string get_cpuHz() {
    ...
    try {
        double hz = 0.0;
        //? Try to get freq from /sys/devices/system/cpu/cpufreq/policy first
        //(faster)
        if (not freq_path.empty()) {
            hz = stod(readfile(freq_path, "0.0")) / 1000;
            if (hz <= 0.0 and ++failed >= 2) freq_path.clear();
        }
        ...
        if (hz >= 1000) {
            if (hz >= 10000)
                cpuhz = to_string((int)round(
                    hz / 1000));  // Future proof until we reach THz speeds :)
            else
                cpuhz = to_string(round(hz / 100) / 10.0).substr(0, 3);
            cpuhz += " GHz";
        } else if (hz > 0)
...
auto get_battery() -> tuple<int, long, string> {
    ...
    if (batteries.empty() and has_battery) {
        if (fs::exists("/sys/class/power_supply")) {
            for (const auto& d :
                ...
                //? see kernel docs for details on the file structure and
                // contents ?
                // https://www.kernel.org/doc/Documentation/ABI/testing/sysfs-class-power
                battery new_bat;
                fs::path bat_dir;
                try {
                    if (not d.is_directory() or
                        not fs::exists(d.path() / "type") or
                    ...
                    //? skip power supplies not conforming to the kernel
                    // standard
                    continue;
                }
                ...
                if (fs::exists(bat_dir / "power_now"))
                    ...
                    new_bat.power_now = bat_dir / "current_now";
    ...
    if (b.use_energy) {
        try {
            percent = round(100.0 * stoll(readfile(b.energy_now, "-1")) /
                            stoll(readfile(b.energy_full, "1")));
    ...
    if (not is_in(status, "charging", "full")) {
        if (b.use_energy and not b.power_now.empty()) {
            try {
                seconds = round((double)stoll(readfile(b.energy_now, "0")) /
                                stoll(readfile(b.power_now, "1")) * 3600);
            } catch (const std::invalid_argument&) {
            } catch (const std::out_of_range&) {
            }
        }
        if (seconds < 0 and fs::exists(b.base_dir / "time_to_empty")) {
            try {
                seconds =
                    stoll(readfile(b.base_dir / "time_to_empty", "0")) * 60;
            } catch (const std::invalid_argument&) {
            } catch (const std::out_of_range&) {
            }
        }
...
auto collect(const bool no_update) -> cpu_info& {
    ...
    try {
        ...
        for (int i = 0; cread.good() and cread.peek() == 'c'; i++) {
            ...
            cread.clear();
            if (times.size() < 4)
                throw std::runtime_error("Malformatted /proc/stat");
            ...
            if (i == 0) {
                const long long calc_totals =
                    max(1ll, totals - cpu_old.at("totals"));
                const long long calc_idles =
                    max(1ll, idles - cpu_old.at("idles"));
                cpu_old.at("totals") = totals;
                cpu_old.at("idles") = idles;

                //? Total usage of cpu
                cpu.cpu_percent.at("total").push_back(
                    clamp((long long)round((double)(calc_totals - calc_idles) *
                                           100 / calc_totals),
                          0ll, 100ll));

                //? Reduce size if there are more values than needed for graph
                while (
                    cmp_greater(cpu.cpu_percent.at("total").size(), width * 2))
                ...
                for (int ii = 0; const auto& val : times) {
                    ...
                    //? Reduce size if there are more values than needed for
                    // graph
                    while (cmp_greater(
                        cpu.cpu_percent.at(time_names.at(ii)).size(),
                        width * 2))
            ...
            //? Calculate cpu total for each core
                ...
                const long long calc_totals =
                    max(0ll, totals - core_old_totals.at(i - 1));
                const long long calc_idles =
    ...
    if (Config::getB("check_temp") and got_sensors) update_sensors();
...
auto collect(const bool no_update) -> mem_info& {
    ...
    if (zfs_arc_cached) {
        ...
        if (arcstats.good()) {
            ...
            while (std::getline(arcstats, line)) {
                std::smatch match;
                if (std::regex_match(line, match, zfs_size_regex) &&
                    match.size() == 2) {
    ...
    if (show_swap and mem.stats.at("swap_total") > 0) {
        for (const auto& name : swap_names) {
            mem.percent.at(name).push_back(round(
                (double)mem.stats.at(name) * 100 / mem.stats.at("swap_total")));
            while (cmp_greater(mem.percent.at(name).size(), width * 2))
                mem.percent.at(name).pop_front();
        }
        has_swap = true;
        ...
        try {
            ...
            if (diskread.good()) {
                ...
                while (not diskread.eof()) {
                    ...
                    if ((not use_fstab and not only_physical) or
                        ...
                        //? Save mountpoint, name, dev path and path to
                        /// sys/block stat file
                        if (not disks.contains(mountpoint)) {
                            ...
                            while (devname.size() >= 2) {
                                if (fs::exists(
                                        ...
                                        ec) and
                                    access(string("/sys/block/" + devname +
                                                  "/stat")
                                               .c_str(),
                                           R_OK) == 0) {
                                    if (c > 0 and
                                        fs::exists("/sys/block/" + devname +
                                                       '/' +
                                                       disks.at(mountpoint)
                                                           .dev.filename()
                                                           .string() +
                ...
                throw std::runtime_error(
                    "Failed to get mounts from /etc/mtab and "
                    "/proc/self/mounts");
            diskread.close();
            ...
            for (auto& [mountpoint, disk] : disks) {
                ...
                if (statvfs64(mountpoint.c_str(), &vfs) < 0) {
                    Logger::warning(
                        "Failed to get disk/partition stats with statvfs() "
                        "for: " +
                        mountpoint);
                    continue;
                }
            ...
            if (swap_disk and has_swap) {
                ...
                disks.at("swap").used_percent =
                    mem.percent.at("swap_used").back();
                disks.at("swap").free_percent =
                    mem.percent.at("swap_free").back();
            }
                ...
                if (diskread.good()) {
                    ...
                    if (disk.io_read.empty())
                        ...
                        disk.io_read.push_back(
                            max((int64_t)0,
                                (sectors_read - disk.old_io.at(0)) * 512));
                    ...
                    if (disk.io_write.empty())
                        ...
                        disk.io_write.push_back(
                            max((int64_t)0,
                                (sectors_write - disk.old_io.at(1)) * 512));
                    ...
                    if (disk.io_activity.empty())
                        ...
                        disk.io_activity.push_back(clamp(
                            (long)round((double)(io_ticks - disk.old_io.at(2)) /
...
int errors = 0;
unordered_flat_map<string, uint64_t> graph_max = {{"download", {}},
                                                  {"upload", {}}};
unordered_flat_map<string, array<int, 2>> max_count = {{"download", {}},
                                                       {"upload", {}}};
bool rescale = true;
...
auto collect(const bool no_update) -> net_info& {
    ...
    if (not no_update and errors < 3) {
        ...
        for (auto* ifa = if_wrap(); ifa != NULL; ifa = ifa->ifa_next) {
            ...
            //? Get IPv4 address
            if (family == AF_INET) {
                if (getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in), ip,
                                NI_MAXHOST, NULL, 0, NI_NUMERICHOST) == 0)
                    net[iface].ipv4 = ip;
            }
            //? Get IPv6 address
            else if (family == AF_INET6) {
                if (getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in6), ip,
                                NI_MAXHOST, NULL, 0, NI_NUMERICHOST) == 0)
                    net[iface].ipv6 = ip;
            }
        ...
        //? Get total recieved and transmitted bytes + device address if no ip
        // was found
        for (const auto& iface : interfaces) {
            ...
            for (const string dir : {"download", "upload"}) {
                ...
                uint64_t val = 0;
                try {
                    val = (uint64_t)stoull(readfile(sys_file, "0"));
                } catch (const std::invalid_argument&) {
                } catch (const std::out_of_range&) {
                }
                ...
                saved_stat.speed =
                    round((double)(val - saved_stat.last) /
    ...
    if (selected_iface.empty() or not v_contains(interfaces, selected_iface)) {
        ...
        if (not config_iface.empty() and v_contains(interfaces, config_iface))
            ...
            rng::sort(sorted_interfaces, [&](const auto& a, const auto& b) {
                return cmp_greater(net.at(a).stat["download"].total +
                                       net.at(a).stat["upload"].total,
                                   net.at(b).stat["download"].total +
                                       net.at(b).stat["upload"].total);
            });
        ...
        for (const auto& dir : {"download", "upload"}) {
            for (const auto& sel : {0, 1}) {
                if (rescale or max_count[dir][sel] >= 5) {
                    const uint64_t avg_speed =
                        (net[selected_iface].bandwidth[dir].size() > 5
                             ? std::accumulate(net.at(selected_iface)
                                                   .bandwidth.at(dir)
                                                   .rbegin(),
                                               net.at(selected_iface)
                                                       .bandwidth.at(dir)
                                                       .rbegin() +
                                                   5,
                                               0) /
                    ...
                    graph_max[dir] =
                        max(uint64_t(avg_speed * (sel == 0 ? 1.3 : 3.0)),
                            (uint64_t)10 << 10);
            ...
            if (sync) {
                const auto other =
                    (string(dir) == "upload" ? "download" : "upload");
...
void _tree_gen(proc_info& cur_proc, vector<proc_info>& in_procs,
               vector<std::reference_wrapper<proc_info>>& out_procs,
               int cur_depth, const bool collapsed, const string& filter,
               bool found = false, const bool no_update = false,
    ...
    if (not found and (should_filter or not filter.empty())) {
        if (not s_contains(std::to_string(cur_proc.pid), filter) and
            not s_contains(cur_proc.name, filter) and
            not s_contains(cur_proc.cmd, filter) and
            not s_contains(cur_proc.user, filter)) {
            filtering = true;
    ...
    //? Set tree index position for process if not filtered out or currently in
    // a collapsed sub-tree
    if (not collapsed and not filtering) {
        out_procs.push_back(std::ref(cur_proc));
        cur_proc.tree_index = out_procs.size() - 1;
        //? Try to find name of the binary file and append to program name if
        // not the same
        if (cur_proc.short_cmd.empty() and not cur_proc.cmd.empty()) {
            std::string_view cmd_view = cur_proc.cmd;
            cmd_view = cmd_view.substr(
                (size_t)0, min(cmd_view.find(' '), cmd_view.size()));
            cmd_view = cmd_view.substr(
                min(cmd_view.find_last_of('/') + 1, cmd_view.size()));
    ...
    int children = 0;
    for (auto& p : rng::equal_range(in_procs, cur_proc.pid, rng::less{},
                                    &proc_info::ppid)) {
        if (not no_update and not filtering and
            (collapsed or cur_proc.collapsed)) {
        ...
        if (collapsed and not filtering) {
            ...
            children++;
        _tree_gen(p, in_procs, out_procs, cur_depth + 1,
                  (collapsed ? true : cur_proc.collapsed), filter, found,
                  no_update, should_filter);
    }
    if (collapsed or filtering) return;

    //? Add tree terminator symbol if it's the last child in a sub-tree
    if (out_procs.size() > cur_pos + 1 and
        not out_procs.back().get().prefix.ends_with("]─"))
        out_procs.back().get().prefix.replace(
            out_procs.back().get().prefix.size() - 8, 8, " └─ ");

    //? Add collapse/expand symbols if process have any children
    out_procs.at(cur_pos).get().prefix =
        " │ "s * cur_depth +
        (children > 0 ? (cur_proc.collapsed ? "[+]─" : "[-]─") : " ├─ ");
}

//* Get detailed info for selected process
void _collect_details(const size_t pid, const uint64_t uptime,
...
auto collect(const bool no_update) -> vector<proc_info>& {
    ...
    //* Use pids from last update if only changing filter, sorting or tree
    // options
    ...
    // start----------------------------------------------
        ...
        for (const auto& d : fs::directory_iterator(Shared::procPath)) {
            ...
            //? Get RSS memory from /proc/[pid]/statm if value from
            /// proc/[pid]/stat looks wrong
            ...
            new_proc.cpu_p =
                clamp(round(cmult * 1000 * (cpu_t - new_proc.cpu_t) /
            ...
            if (show_detailed and not got_detailed and
                new_proc.pid == detailed_pid) {
                got_detailed = true;
            }
        }
    ...
    if (sorted_change or not no_update) {
        if (reverse) {
            switch (v_index(sort_vector, sorting)) {
                case 0:
                    rng::stable_sort(current_procs, rng::less{},
                                     &proc_info::pid);
                    break;
                case 1:
                    rng::stable_sort(current_procs, rng::less{},
            ...
            switch (v_index(sort_vector, sorting)) {
                case 0:
                    rng::stable_sort(current_procs, rng::greater{},
                                     &proc_info::pid);
                    break;
                case 1:
                    rng::stable_sort(current_procs, rng::greater{},
        ...
        //* When sorting with "cpu lazy" push processes over threshold cpu usage
        // to the front regardless of cumulative usage
        if (not tree and not reverse and sorting == "cpu lazy") {
            double max = 10.0, target = 30.0;
            for (size_t i = 0, x = 0, offset = 0; i < current_procs.size();
                 i++) {
                ...
                if (i == offset and current_procs.at(i).cpu_p > 30.0)
                    ...
                    rotate(current_procs.begin() + offset,
                           current_procs.begin() + i,
                           current_procs.begin() + i + 1);
                    if (++x > 10) break;
                }
            }
        }
    ...
    if (should_filter) {
        ...
        for (auto& p : current_procs) {
            if (not tree and not filter.empty()) {
                if (not s_contains(to_string(p.pid), filter) and
                    not s_contains(p.name, filter) and
                    not s_contains(p.cmd, filter) and
                    not s_contains(p.user, filter)) {
                    p.filtered = true;
                    filter_found++;
                } else {
                    p.filtered = false;
    ...
    if (tree and (not no_update or should_filter or sorted_change)) {
        if (auto find_pid = (collapse != -1 ? collapse : expand);
            find_pid != -1) {
            auto collapser =
                rng::find(current_procs, find_pid, &proc_info::pid);
...
double system_uptime() {
    ...
    throw std::runtime_error("Failed get uptime from from " +
                             (string)Shared::procPath + "/uptime");
}
}  // namespace Tools
