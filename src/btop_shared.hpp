...
#if defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__NetBSD__)
	struct KvmDeleter {
		void operator()(kvm_t* handle) { kvm_close(handle); }
	};
...
namespace Gpu {
	...
	struct gpu_info_supported {
		bool gpu_utilization = true, mem_utilization = true, gpu_clock = true, mem_clock = true, pwr_usage = true,
			 pwr_state = true, temp_info = true, mem_total = true, mem_used = true, pcie_txrx = true,
			 encoder_utilization = true, decoder_utilization = true;
	};
...
namespace Cpu {
	...
	string
	draw(const cpu_info& cpu, const vector<Gpu::gpu_info>& gpu, bool force_redraw = false, bool data_same = false);
