...
namespace Draw {
    ...
    string banner_gen(int y, int x, bool centered, bool redraw) {
        ...
        if (banner.empty()) {
            ...
            for (size_t z = 0; const auto& line : Global::Banner_src) {
                if (const auto w = ulen(line[1]); w > width)
                    width = w;
                if (tty_mode) {
                ...
                if (++z < Global::Banner_src.size())
                    banner += Mv::l(ulen(line[1])) + Mv::d(1);
            }
            banner +=
                Mv::r(18 - Global::Version.size()) + Theme::c("main_fg") + Fx::b + Fx::i + "v" + Global::Version + Fx::reset;
        }
        if (redraw)
            return "";
    ...
    string createBox(
        ...
        const string numbering =
            (num == 0) ? "" : Theme::c("hi_fg") + (tty_mode ? std::to_string(num) : Symbols::superscript.at(clamp(num, 0, 9)));
        ...
        for (const int& hpos : iota(y + 1, y + height - 1)) {
            out += Mv::to(hpos, x) + Symbols::v_line + ((fill) ? string(width - 2, ' ') : Mv::r(width - 2)) + Symbols::v_line;
        }
    ...
    bool update_clock(bool force) {
        ...
        if (not Cpu::shown or clock_format.empty()) {
            if (clock_format.empty() and not Global::clock.empty())
                Global::clock.clear();
            return false;
        }

        static const std::unordered_map<string, string> clock_custom_format = { { "/user", Tools::username() },
                                                                                { "/host", Tools::hostname() },
                                                                                { "/uptime", "" } };
        ...
        clock_str = uresize(
            clock_str,
            std::max(10, width - 66 - (Term::width >= 100 and Config::getB("show_battery") and Cpu::has_battery ? 22 : 0))
        );
        out.clear();
        ...
        out += Mv::to(y, x + (width / 2) - (clock_len / 2)) + Fx::ub + Theme::c("cpu_box") + title_left + Theme::c("title") +
               Fx::b + clock_str + Theme::c("cpu_box") + Fx::ub + title_right;
    ...
    string Meter::operator()(int value) {
        ...
        if (not cache.at(value).empty())
            return cache.at(value);
...
namespace Cpu {
    ...
    string draw(const cpu_info& cpu, const vector<Gpu::gpu_info>& gpus, bool force_redraw, bool data_same) {
        ...
        auto& graph_bg =
            Symbols::graph_symbols.at((graph_symbol == "default" ? Config::getS("graph_symbol") + "_up" : graph_symbol + "_up"))
                .at(6);
        ...
        if (redraw) {
            ...
            //? Buttons on title
            out += Mv::to(button_y, x + 10) + title_left + Theme::c("hi_fg") + Fx::b + 'm' + Theme::c("title") + "enu" + Fx::ub +
                   title_right;
            ...
            auto init_graphs = [&](vector<Draw::Graph>& graphs,
                ...
                if (graph_field.starts_with("gpu")) {
                    if (graph_field.find("totals") != string::npos) {
                        ...
                        for (size_t i = 0; i < gpus.size(); i++) {
                            ...
                            if (gpu.supported_functions.gpu_utilization) {
                                if (i + 1 < gpus.size()) {
                                    graph =
                                        Draw::Graph { graph_width,  graph_height, "cpu", safeVal(gpu.gpu_percent, graph_field),
                                                      graph_symbol, invert,       true };
                                } else {
                                    graph = Draw::Graph { graph_width + graph_default_width % graph_width - (int)gpus.size() + 1,
                                                          graph_height,
                                                          "cpu",
                        ...
                        graphs[0] =
                            Draw::Graph { graph_width,  graph_height, "cpu", safeVal(Gpu::shared_gpu_percent, graph_field),
                                          graph_symbol, invert,       true };
                    }
                } else {
#endif
                    graphs.resize(1);
                    graph_width = graph_default_width;
                    graphs[0] = Draw::Graph { graph_width,  graph_height, "cpu", safeVal(cpu.cpu_percent, graph_field),
                                              graph_symbol, invert,       true };
#ifdef GPU_SUPPORT
                }
#endif
            ...
            if (show_gpu and b_columns > 1) {
                ...
                for (size_t i = 0; i < gpus.size(); i++) {
                    ...
                    if (gpu.supported_functions.temp_info and show_temps) {
                        gpu_temp_graphs[i] =
                            Draw::Graph { gpu_graph_width, 1, "temp", gpu.temp, graph_symbol, false, false, gpu.temp_max, -23 };
                        width_left -= 11;
                    }
        ...
        if (Config::getB("show_battery") and has_battery) {
            ...
            if (redraw or percent != old_percent or (watts != old_watts and Config::getB("show_battery_watts")) or
                ...
                const int current_len = (Term::width >= 100 ? 11 : 0) + str_time.size() + str_percent.size() + str_watts.size() +
                                        to_string(Config::getI("update_ms")).size();
                ...
                out += Mv::to(y, bat_pos) + title_left + Theme::c("title") + Fx::b + "BAT" + bat_symbol + ' ' + str_percent +
                       ...
                       (not str_watts.empty() ? " " + Theme::c("title") + Fx::b + str_watts : "") + Fx::ub + title_right;
            }
        } else if (bat_pos > 0) {
            out += Mv::to(y, bat_pos) + Fx::ub + Theme::c("cpu_box") + Symbols::h_line * (bat_len + 4);
            bat_pos = bat_len = 0;
        ...
        try {
            ...
            auto draw_graphs =
                [&](vector<Draw::Graph>& graphs, const int graph_height, const int graph_width, const string& graph_field) {
#ifdef GPU_SUPPORT
                    if (graph_field.starts_with("gpu"))
                        if (graph_field.ends_with("totals")) {
                            ...
                            for (size_t i = 0; i < gpus.size(); i++) {
                                ...
                                if (Gpu::count - (gpu_auto ? Gpu::shown : 0) > 1) {
                                    ...
                                    out += Mv::l(graph_width - 1) + Mv::u(graph_height / 2) + (graph_width > 5 ? "GPU" : "") +
                                           i_str + Mv::d(graph_height / 2) +
            ...
            if (show_watts) {
                ...
                out += Theme::g("cached").at(clamp(cpu.usage_watts / max_observed_pwr * 100.0f, 0.0f, 100.0f)) + cwatts +
                       Theme::c("main_fg") + cwatts_post;
            }
        ...
        for (const auto& n : iota(0, Shared::coreCount)) {
            ...
            if (show_temps and not hide_cores) {
                ...
                const auto temp_color =
                    enabled ? Theme::g("temp").at(clamp(safeVal(cpu.temp, n + 1).back() * 100 / cpu.temp_max, 0ll, 100ll))
                            : Theme::c("inactive_fg");
        ...
        if (show_gpu) {
            for (unsigned long i = 0; i < gpus.size(); ++i) {
                ...
                if (gpus.size() > 1)
                    out += rjust(to_string(i), 1 + (gpus.size() > 9));
    ...
    string draw(const gpu_info& gpu, unsigned long index, bool force_redraw, bool data_same) {
        ...
        if (redraw[index]) {
            ...
            if (gpu.supported_functions.pwr_usage)
                pwr_meter =
                    Draw::Meter { b_width - (gpu.supported_functions.pwr_state and gpu.pwr_state != 32 ? 25 : 12), "cached" };
            ...
            if (gpu.supported_functions.mem_used and gpu.supported_functions.mem_total)
                mem_used_graph = Draw::Graph { b_width / 2 - 2,
        ...
        if (gpu.supported_functions.gpu_clock) {
            ...
            out += Mv::to(b_y, b_x + b_width - 12) + Theme::c("div_line") + Symbols::h_line * (5 - clock_speed_string.size()) +
                   Symbols::title_left + Fx::b + Theme::c("title") + clock_speed_string + " MHz" + Fx::ub + Theme::c("div_line") +
                   Symbols::title_right;
        }
        ...
        if (drawnEncDec) {
            out += Mv::to(b_y + rows_used, b_x + 1) + Theme::c("main_fg") + Fx::b + "ENC " + enc_meter(gpu.encoder_utilization) +
                   ...
                   Theme::c("main_fg") + '%' + Theme::c("div_line") + Symbols::v_line + Theme::c("main_fg") + Fx::b + "DEC " +
                   enc_meter(gpu.decoder_utilization) + Theme::g("cpu").at(clamp(gpu.decoder_utilization, 0ll, 100ll)) +
        ...
        if (gpu.supported_functions.mem_total or gpu.supported_functions.mem_used) {
            ...
            if (gpu.supported_functions.mem_total and gpu.supported_functions.mem_used) {
                ...
                out += Theme::c("div_line") + Symbols::div_left + Symbols::h_line + Symbols::title_left + Fx::b +
                       Theme::c("title") + "vram" + Theme::c("div_line") + Fx::ub + Symbols::title_right +
                       Symbols::h_line * (b_width / 2 - 8) + Symbols::div_up + Mv::d(offset) + Mv::l(1) + Symbols::div_down +
                       Mv::l(1) + Mv::u(1) + (Symbols::v_line + Mv::l(1) + Mv::u(1)) * (offset - 1) + Symbols::div_up +
                       ...
                       Mv::l(b_width - 3) + Mv::u(1 + 2 * gpu.supported_functions.mem_utilization) + Theme::c("main_fg") + Fx::b +
                       "Total:" + rjust(floating_humanizer(gpu.mem_total), b_width / 2 - 9) + Fx::ub + Mv::r(3) +
                       rjust(to_string(safeVal(gpu.gpu_percent, "gpu-vram-totals"s).back()), 3) + '%';

                //? Memory utilization
                if (gpu.supported_functions.mem_utilization)
                    out += Mv::l(b_width / 2 + 6) + Mv::d(1) + Theme::c("div_line") + Symbols::div_left + Symbols::h_line +
                           Theme::c("title") + "Utilization:" + Theme::c("div_line") + Symbols::h_line * (b_width / 2 - 14) +
                           Symbols::div_right + Mv::l(b_width / 2) + Mv::d(1) +
                           mem_util_graph(gpu.mem_utilization_percent, (data_same or redraw[index])) + Mv::l(b_width / 2 - 1) +
                           Mv::u(1) + rjust(to_string(gpu.mem_utilization_percent.back()), 3) + '%';
                ...
                if (gpu.supported_functions.mem_total)
                    out += "VRAM total:" +
                           rjust(floating_humanizer(gpu.mem_total), b_width / (1 + gpu.supported_functions.mem_clock) - 14);
                else
                    out += "VRAM usage:" +
        ...
        if (gpu.supported_functions.pcie_txrx and Config::getB("nvml_measure_pcie_speeds")) {
            ...
            out +=
                Mv::to(b_y + b_height_vec[index] - 1, b_x + 2) + Theme::c("div_line") + Symbols::title_left_down +
                Theme::c("title") + Fx::b + "TX:" + Fx::ub + Theme::c("div_line") + Symbols::title_right_down +
                Symbols::h_line * (b_width / 2 - 9 - tx_string.size()) + Symbols::title_left_down + Theme::c("title") + Fx::b +
                tx_string + Fx::ub + Theme::c("div_line") + Symbols::title_right_down +
                (gpu.supported_functions.mem_total and gpu.supported_functions.mem_used ? Symbols::div_down : Symbols::h_line) +
...
namespace Mem {
    ...
    string draw(const mem_info& mem, bool force_redraw, bool data_same) {
        ...
        if (redraw) {
            ...
            if (show_swap and has_swap) {
                for (const auto& name : swap_names) {
                    if (use_graphs)
                        mem_graphs[name] =
                            Draw::Graph { mem_meter, graph_height, name.substr(5), safeVal(mem.percent, name), graph_symbol };
                    else
            ...
            if (show_disks) {
                    ...
                    for (const auto& [name, disk] : mem.disks) {
                        ...
                        if (io_mode) {
                            ...
                            if (io_graph_combined) {
                                ...
                                io_graphs[name] = Draw::Graph { disks_width,  disks_io_h, "available", combined,
                                                                graph_symbol, false,      true,        speed };
                            } else {
                                io_graphs[name + "_read"] = Draw::Graph { disks_width,  half_height, "free", disk.io_read,
                                                                          graph_symbol, false,       true,   speed };
                                io_graphs[name + "_write"] = Draw::Graph {
                                    disks_width, disks_io_h - half_height, "used", disk.io_write, graph_symbol, true, true, speed
                                };
                            }
                        }
                    }
                ...
                out += Mv::to(y, x + width - 6) + Fx::ub + Theme::c("mem_box") + Symbols::title_left + (io_mode ? Fx::b : "") +
                       Theme::c("hi_fg") + 'i' + Theme::c("title") + 'o' + Fx::ub + Theme::c("mem_box") + Symbols::title_right;
        ...
        string divider =
            (graph_height > 0
                 ? Mv::l(2) + Theme::c("mem_box") + Symbols::div_left + Theme::c("div_line") + Symbols::h_line * (mem_width - 1) +
        ...
        out += Mv::to(y + 1, x + 2) + Theme::c("title") + Fx::b + "Total:" + rjust(floating_humanizer(totalMem), mem_width - 9) +
        ...
        if (show_disks) {
            ...
            divider = Mv::l(1) + Theme::c("div_line") + Symbols::div_left + Symbols::h_line * disks_width + Theme::c("mem_box") +
            ...
            if (io_mode) {
                for (const auto& mount : mem.disks_order) {
                    ...
                    out += Mv::to(y + 1 + cy, x + 1 + cx) + divider + Theme::c("title") + Fx::b +
                           uresize(disk.name, disks_width - 8) + Mv::to(y + 1 + cy, x + cx + disks_width - total.size()) +
                           trans(total) + Fx::ub;
                    if (big_disk) {
                    ...
                    if (io_graphs.contains(mount + "_activity")) {
                        out += Mv::to(y + 2 + cy++, x + 1 + cx) + (big_disk ? " IO% " : " IO   " + Mv::l(2)) +
                               ...
                               io_graphs.at(mount + "_activity")(disk.io_activity, redraw or data_same) + Theme::c("main_fg");
                    }
                    if (++cy > height - 3)
                        break;
                    if (io_graph_combined) {
                        ...
                        if (disks_io_h <= 3)
                            out += Mv::to(y + 1 + cy, x + 1 + cx) + string(5, ' ') + Mv::to(y + cy + disks_io_h, x + 1 + cx) +
                                   string(5, ' ');
                        out +=
                            Mv::to(y + 1 + cy, x + 1 + cx) + io_graphs.at(mount + "_read")(disk.io_read, redraw or data_same) +
                            Mv::l(disks_width) + Mv::d(1) + io_graphs.at(mount + "_write")(disk.io_write, redraw or data_same) +
                ...
                for (const auto& mount : mem.disks_order) {
                    ...
                    const string human_io =
                        (comb_val > 0
                             ? (disk.io_write.back() > 0 and big_disk ? "▼"s : ""s) +
                                   (disk.io_read.back() > 0 and big_disk ? "▲"s : ""s) + floating_humanizer(comb_val, true)
                             : "");
                    ...
                    out += Mv::to(y + 1 + cy, x + 1 + cx) + divider + Theme::c("title") + Fx::b +
                           uresize(disk.name, disks_width - 8) + Mv::to(y + 1 + cy, x + cx + disks_width - human_total.size()) +
                           trans(human_total) + Fx::ub + Theme::c("main_fg");
                    if (big_disk and not human_io.empty())
                        out +=
                            Mv::to(
                                y + 1 + cy, x + 1 + cx + round((double)disks_width / 2) - round((double)human_io.size() / 2) - 1
                            ) +
                    ...
                    if (show_io_stat and io_graphs.contains(mount + "_activity")) {
                        out += Mv::to(y + 1 + cy, x + 1 + cx) + (big_disk ? " IO% " : " IO   " + Mv::l(2)) +
                               ...
                               Theme::g("available").at(clamp(disk.io_activity.back(), 50ll, 100ll)) + Mv::l(disks_width - 6) +
                               io_graphs.at(mount + "_activity")(disk.io_activity, redraw or data_same) + Theme::c("main_fg");
                        if (not big_disk)
                            out += Mv::to(y + 1 + cy, x + cx + 1) + Theme::c("main_fg") + human_io;
                        if (++cy > height - 3)
                            break;
                    }
                    ...
                    if (disk_meters_free.contains(mount) and
                        ...
                        cy++;
                        if (cmp_less_equal(disks.size() * 4 + (show_io_stat ? disk_ios : 0), height - 1))
                            cy++;
                    }
                }
            }
    ...
    string draw(const net_info& net, bool force_redraw, bool data_same) {
        ...
        if (redraw) {
            ...
            if (safeVal(net.bandwidth, "download"s).empty() or safeVal(net.bandwidth, "upload"s).empty())
                return out + Fx::reset;
            graphs["download"] = Draw::Graph { width - b_width - 2, u_graph_height, "download", net.bandwidth.at("download"),
                                               graph_symbol,        false,          true,       down_max };
            graphs["upload"] = Draw::Graph { width - b_width - 2, d_graph_height, "upload", net.bandwidth.at("upload"),
                                             graph_symbol,        true,           true,     up_max };

            //? Interface selector and buttons

            out += Mv::to(y, x + width - i_size - 9) + title_left + Fx::b + Theme::c("hi_fg") + Symbols::left + "b " +
                   Theme::c("title") + uresize(selected_iface, MAX_IFNAMSIZ) + Theme::c("hi_fg") + " n" + Symbols::right +
                   title_right + Mv::to(y, x + width - i_size - 15) + title_left + Theme::c("hi_fg") +
                   (safeVal(net.stat, "download"s).offset + safeVal(net.stat, "upload"s).offset > 0 ? Fx::b : "") + 'z' +
            ...
            if (width - i_size - 20 > 6) {
                out += Mv::to(y, x + width - i_size - 21) + title_left + Theme::c("hi_fg") + (net_auto ? Fx::b : "") + 'a' +
                       Theme::c("title") + "uto" + title_right;
                Input::mouse_mappings["a"] = { y, x + width - i_size - 20, 1, 4 };
            }
            if (width - i_size - 20 > 13) {
                out += Mv::to(y, x + width - i_size - 27) + title_left + Theme::c("title") + (net_sync ? Fx::b : "") + 's' +
...
namespace Proc {
    ...
    int selection(const std::string_view cmd_key) {
        ...
        const int select_max =
            (Config::getB("show_detailed") ? (Config::getB("pause_proc_list") ? Proc::select_max - 9 : Proc::select_max - 8)
        ...
        if ((cmd_key == "up" or (vim_keys and cmd_key == "k")) and selected > 0) {
            ...
            start = clamp(
                (int)round((double)mouse_y * (numpids - select_max - 2) / (select_max - 2)), 0, max(0, numpids - select_max)
            );
        }
    ...
    string draw(const vector<proc_info>& plist, bool force_redraw, bool data_same) {
        ...
        if (redraw) {
            ...
            if (show_detailed) {
                ...
                if (alive or pause_proc_list) {
                    ...
                    detailed_mem_graph =
                        Draw::Graph { d_width / 3, 1, "", detailed.mem_bytes, graph_symbol, false, false, detailed.first_mem };
                }
                ...
                out += Mv::to(y, x) + Theme::c("proc_box") + Symbols::div_left + Symbols::h_line + title_left +
                       Theme::c("hi_fg") + Fx::b + (tty_mode ? "4" : Symbols::superscript.at(4)) + Theme::c("title") + "proc" +
                       Fx::ub + title_right + Symbols::h_line * (width - 10) + Symbols::div_right + Mv::to(d_y, dgraph_x + 2) +
                       title_left + Fx::b + Theme::c("title") + pid_str + Fx::ub + title_right + title_left + Fx::b +
                       Theme::c("title") + uresize(detailed.entry.name, dgraph_width - pid_str.size() - 7, true) + Fx::ub +
                       title_right;

                out += Mv::to(d_y, d_x - 1) + Theme::c("proc_box") + Symbols::div_up + Mv::to(y, d_x - 1) + Symbols::div_down +
                       Theme::c("div_line");
                ...
                out += title_left + hi_color + Fx::b + (vim_keys ? 'K' : 'k') + t_color + "ill" + Fx::ub + title_right +
                       title_left + hi_color + Fx::b + 's' + t_color + "ignals" + Fx::ub + title_right + title_left + hi_color +
                       Fx::b + 'N' + t_color + "ice" + Fx::ub + title_right;
                if (alive and selected == 0) {
            ...
            out += Mv::to(y, sort_pos) + title_left + Fx::b + Theme::c("hi_fg") + Symbols::left + " " + Theme::c("title") +
                   sorting + " " + Theme::c("hi_fg") + Symbols::right + Fx::ub + title_right;
            ...
            const string down_button =
                (selected == select_max and start == numpids - select_max ? Theme::c("inactive_fg") : Theme::c("hi_fg")) +
                Symbols::down;
            ...
            int mouse_x = x + 14;
            out += Mv::to(y + height - 1, x + 1) + title_left_down + Fx::b + hi_color + Symbols::up + Theme::c("title") +
                   " select " + down_button + Fx::ub + title_right_down + title_left_down + Fx::b + t_color + "info " + hi_color +
                   Symbols::enter + Fx::ub + title_right_down;
            if (selected > 0)
            ...
            if (selected > 0)
                Input::mouse_mappings["N"] = { y + height - 1, mouse_x, 1, 5 };

            //? Labels for fields in list
            if (not proc_tree)
                out += Mv::to(y + 1, x + 1) + Theme::c("title") + Fx::b + rjust("Pid:", 8) + ' ' + ljust("Program:", prog_size) +
        ...
        if (show_detailed) {
            ...
            out += Mv::to(d_y + 1, dgraph_x + 1) + Fx::ub +
                   detailed_cpu_graph(detailed.cpu_percent, (redraw or data_same or not alive)) + Mv::to(d_y + 1, dgraph_x + 1) +
                   Theme::c("title") + Fx::b + rjust(cpu_str, 4) + "%";
            ...
            out += Mv::to(d_y + 2, d_x + 1) + stat_color + Fx::ub + cjust(detailed.status, item_width) + Theme::c("main_fg") +
            ...
            out += Mv::to(d_y + 4, d_x + 1) + Theme::c("title") + Fx::b +
                   ...
                   Theme::c("inactive_fg") + Fx::ub + graph_bg * (d_width / 3) + Mv::l(d_width / 3) + Theme::c("proc_misc") +
                   detailed_mem_graph(detailed.mem_bytes, (redraw or data_same or not alive)) + ' ' + Theme::c("title") + Fx::b +
                   detailed.memory;
        }
        ...
        for (int n = 0; auto& p : plist) {
            if (p.filtered or (proc_tree and p.tree_index == plist.size()) or n++ < start)
                continue;
            ...
            if (not p_wide_cmd.contains(p.pid))
                p_wide_cmd[p.pid] = ulen(san_cmd) != ulen(san_cmd, true);
            ...
            out +=
                ...
                ljust((cmp_greater(p.user.size(), user_size) ? p.user.substr(0, user_size - 1) + '+' : p.user), user_size) + ' ' +
                m_color + rjust(mem_str, 5) + end + ' ' + (is_selected ? "" : Theme::c("inactive_fg")) +
                (show_graphs ? graph_bg * 5 : "") +
                (p_graphs.contains(p.pid)
                     ? Mv::l(5) + c_color +
                           p_graphs.at(p.pid)({ (p.cpu_p >= 0.1 and p.cpu_p < 5 ? 5ll : (long long)round(p.cpu_p)) }, data_same)
                     : "") +
        ...
        out += Mv::to(y + height - 1, x + width - 3 - max(9, (int)location.size())) + Fx::ub + Theme::c("proc_box") + loc_clear +
               Symbols::title_left_down + Theme::c("title") + Fx::b + location + Fx::ub + Theme::c("proc_box") +
               Symbols::title_right_down;
...
namespace Draw {
    void calcSizes() {
        ...
        if (Mem::shown) {
            ...
            height =
                ceil(
                    (double)Term::height * (100 - Net::height_p * Net::shown * 4 / ((Gpu::shown != 0 and Cpu::shown) + 4)) / 100
                ) -
            ...
            if (mem_graphs) {
                graph_height =
                    max(1,
                        (int)round(
                            (double)((height - (has_swap and not swap_disk ? 2 : 1)) - (mem_size == 3 ? 2 : 1) * item_height) /
                            item_height
                        ));
                if (graph_height > 1)
                    mem_meter += 6;
