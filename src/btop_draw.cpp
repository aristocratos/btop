    ...
    string draw(const cpu_info& cpu, const vector<Gpu::gpu_info>& gpus, bool force_redraw, bool data_same) {
		...
		if (show_watts) {
			...
			fmt::format_to(std::back_inserter(out), " {watts_color}{cwatts:>4.{precision}f}{main_fg}W",
				"watts_color"_a = Theme::g("cached").at(clamp(clamped_watts / max_observed_pwr * 100.0f, 0.0f, 100.0f)),
				"cwatts"_a = clamped_watts,
				"precision"_a = clamped_watts < 9.995f ? 2 : clamped_watts < 99.95f ? 1 : 0,
