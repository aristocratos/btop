...
namespace Cpu {
    ...
    string draw(
		...
		if (redraw) {
			...
			const bool is_focused_on_proc = (Config::getB("follow_process") and Config::getI("followed_pid") == Config::getI("detailed_pid")) or Config::getI("proc_selected") > 0;
			out += box;
			...
			out += Mv::to(button_y, x + width - update.size() - 8) + title_left + Fx::b + Theme::c(is_focused_on_proc and Config::getB("proc_tree") ? "inactive_fg" : "hi_fg") + "- " + Theme::c(is_focused_on_proc and Config::getB("proc_tree") ? "inactive_fg" : "title") + update
				+ Theme::c(is_focused_on_proc and Config::getB("proc_tree") ? "inactive_fg" : "hi_fg") + " +" + Fx::ub + title_right;
