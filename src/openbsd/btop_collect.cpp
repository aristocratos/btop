...
#define MUKTOC(v) ((v - 273150000) / 1000000.0)
	...
	auto collect(bool no_update) -> cpu_info& {
		...
		auto cp_time = std::unique_ptr<struct cpustats[]> {new struct cpustats[Shared::coreCount]};
	...
	void collect_disk(std::unordered_map<string, disk_info>& disks, std::unordered_map<string, string>& mapping) {
		...
		if (sysctl(mib, 2, &num_drives, &size, NULL, 0) >= 0) {
			...
			auto p = std::unique_ptr<struct diskstats[], void (*)(void*)> {
				reinterpret_cast<struct diskstats*>(malloc(size)), free
			};
...
namespace Proc {
	...
	auto collect(bool no_update) -> vector<proc_info>& {
		...
		if (no_update and not current_procs.empty()) {
			...
			const struct kinfo_proc* kprocs =
				kvm_getprocs(kd.get(), KERN_PROC_ALL, 0, sizeof(struct kinfo_proc), &count);
