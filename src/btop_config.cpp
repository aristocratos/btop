...
namespace Config {
	...
	const vector<array<string, 2>> descriptions = {
		...
		{"disks_filter", 		"#* Optional filter for shown disks, should be full path of a mountpoint, separate multiple values with whitespace \" \".\n"
									"#* If the line starts with \033[3mexclude=\033[0m disks matching the filter will \033[3mnot\033[0m be shown. Otherwise only the disks matching the filter will be shown. Examples: disks_filter=\"exclude=/boot /home/user\"., disk_filter=\"/boot /home/user\""},
