...
namespace Config {
  ...
  const vector<array<string, 2>> descriptions = {
      ...
      {"theme_background",
       "#* If the theme set background should be shown, set to False if you "
       "want "
      ...
      {"vim_keys",
       "#* Set to True to enable \"h,j,k,l,g,G\" keys for directional control "
       "in "
       "lists.\n"
      ...
      {"graph_symbol",
       "#* Default symbols to use for graph creation, \"braille\", \"block\" "
       "or "
       "\"tty\".\n"
      ...
      {"shown_boxes",
       "#* Manually set which boxes to show. Available values are \"cpu mem "
       "net "
       "proc\", separate values with whitespace."},

      {"update_ms",
       "#* Update time in milliseconds, recommended 2000 ms or above for "
       "better "
      ...
      {"proc_info_smaps",
       "#* Use /proc/[pid]/smaps for memory information in the process info "
       "box "
      ...
      {"cpu_graph_upper",
       "#* Sets the CPU stat shown in upper half of the CPU graph, \"total\" "
       "is "
       "always available.\n"
       "#* Select from a list of detected attributes from the options menu."},

      {"cpu_graph_lower",
       "#* Sets the CPU stat shown in lower half of the CPU graph, \"total\" "
       "is "
       "always available.\n"
       "#* Select from a list of detected attributes from the options menu."},
      ...
      {"cpu_core_map",
       ...
       "#* Format \"x:y\" x=core with wrong temp, y=core with correct temp, "
       "use "
      ...
      {"disks_filter",
       "#* Optional filter for shown disks, should be full path of a "
       "mountpoint, "
      ...
      {"show_io_stat",
       "#* Toggles if io activity % (disk busy time) should be shown in "
       "regular "
       "disk usage view."},
      ...
      {"net_auto",
       "#* Use network graphs auto rescaling mode, ignores any values set "
       "above "
       "and rescales down to 10 Kibibytes at the lowest."},

      {"net_sync",
       "#* Sync the auto scaling for download and upload to whichever "
       "currently "
      ...
      {"log_level",
       ...
       "\"WARNING\" \"INFO\" \"DEBUG\".\n"
       "#* The level set includes all lower levels, i.e. \"DEBUG\" will show "
       "all "
       "logging info."}};
	...
	if (cread.good()) {
	  ...
	  while (not cread.eof()) {
		...
		if (bools.contains(name)) {
		  ...
		  if (not isint(value))
			load_warnings.push_back(
			    "Got an invalid integer value for config name: " + name);
