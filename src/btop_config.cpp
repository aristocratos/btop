...
const vector<array<string, 2>> descriptions = {
    {"color_theme",
     ...
     "\"Default\" and \"TTY\" for builtin themes.\n"
     "#* Themes should be placed in \"../share/btop/themes\" relative to "
     "binary or \"$HOME/.config/btop/themes\""},

    {"theme_background",
     "#* If the theme set background should be shown, set to False if you want "
     "terminal background transparency."},

    {"truecolor",
     "#* Sets if 24-bit truecolor should be used, will convert 24-bit colors "
     "to 256 color (6x6x6 color cube) if false."},

    {"force_tty",
     "#* Set to true to force tty mode regardless if a real tty has been "
     "detected or not.\n"
     "#* Will force 16-color mode and TTY theme, set all graph symbols to "
     "\"tty\" and swap out other non tty friendly symbols."},

    {"presets",
     "#* Define presets for the layout of the boxes. Preset 0 is always all "
     "boxes shown with default settings. Max 9 presets.\n"
     "#* Format: \"box_name:P:G,box_name:P:G\" P=(0 or 1) for alternate "
     "positions, G=graph symbol to use for box.\n"
    ...
    {"graph_symbol",
     "#* Default symbols to use for graph creation, \"braille\", \"block\" or "
     "\"tty\".\n"
     "#* \"braille\" offers the highest resolution but might not be included "
     "in all fonts.\n"
     "#* \"block\" has half the resolution of braille but uses more common "
     "characters.\n"
     "#* \"tty\" uses only 3 different symbols but will work with most fonts "
     "and should work in a real TTY.\n"
     "#* Note that \"tty\" only has half the horizontal resolution of the "
     "other two, so will show a shorter historical view."},

    {"graph_symbol_cpu",
     "# Graph symbol to use for graphs in cpu box, \"default\", \"braille\", "
     "\"block\" or \"tty\"."},
    ...
    {"shown_boxes",
     "#* Manually set which boxes to show. Available values are \"cpu mem net "
     "proc\", separate values with whitespace."},

    {"update_ms",
     "#* Update time in milliseconds, recommended 2000 ms or above for better "
    ...
    {"proc_sorting",
     ...
     "direct\" updates top process directly."},
    ...
    {"proc_per_core",
     "#* If process cpu usage should be of the core it's running on or usage "
     "of the total available cpu power."},

    {"proc_mem_bytes", "#* Show process memory as bytes instead of percent."},

    {"proc_info_smaps",
     "#* Use /proc/[pid]/smaps for memory information in the process info box "
     "(very slow but more accurate)"},

    {"proc_left", "#* Show proc box on left side of screen instead of right."},
    {"proc_filter_kernel",
     "#* Filter kernel processes to make the dumps more readable"},

    {"cpu_graph_upper",
     "#* Sets the CPU stat shown in upper half of the CPU graph, \"total\" is "
     "always available.\n"
     "#* Select from a list of detected attributes from the options menu."},

    {"cpu_graph_lower",
     "#* Sets the CPU stat shown in lower half of the CPU graph, \"total\" is "
     "always available.\n"
     "#* Select from a list of detected attributes from the options menu."},
    ...
    {"cpu_sensor",
     "#* Which sensor to use for cpu temperature, use options menu to select "
     "from list of available sensors."},

    {"show_coretemp",
     "#* Show temperatures for cpu cores also if check_temp is True and "
     "sensors has been found."},

    {"cpu_core_map",
     "#* Set a custom mapping between core and coretemp, can be needed on "
     "certain cpus to get correct temperature for correct core.\n"
     "#* Use lm-sensors or similar to see which cores are reporting "
     "temperatures on your machine.\n"
     "#* Format \"x:y\" x=core with wrong temp, y=core with correct temp, use "
     "space as separator between multiple entries.\n"
     "#* Example: \"4:0 5:1 6:3\""},

    {"temp_scale",
     "#* Which temperature scale to use, available values: \"celsius\", "
    ...
    {"clock_format",
     "#* Draw a clock at top of screen, formatting according to strftime, "
     "empty string to disable.\n"
     "#* Special formatting: /host = hostname | /user = username | /uptime = "
     "system uptime"},

    {"background_update",
     "#* Update main ui in background when menus are showing, set this to "
     "false if the menus is flickering too much for comfort."},

    {"custom_cpu_name", "#* Custom cpu model name, empty string to disable."},

    {"disks_filter",
     "#* Optional filter for shown disks, should be full path of a mountpoint, "
     "separate multiple values with whitespace \" \".\n"
     "#* Begin line with \"exclude=\" to change to exclude filter, otherwise "
     "defaults to \"most include\" filter. Example: "
     "disks_filter=\"exclude=/boot /home/user\"."},
    ...
    {"swap_disk",
     "#* Show swap as a disk, ignores show_swap value above, inserts itself "
     "after first disk."},

    {"show_disks", "#* If mem box should be split to also show disks info."},

    {"only_physical",
     "#* Filter out non physical disks. Set this to False to include network "
     "disks, RAM disks and similar."},
    ...
    {"show_io_stat",
     "#* Toggles if io activity % (disk busy time) should be shown in regular "
     "disk usage view."},

    {"io_mode",
     "#* Toggles io mode for disks, showing big graphs for disk read/write "
     "speeds."},
    ...
    {"io_graph_speeds",
     "#* Set the top speed for the io graphs in MiB/s (100 by default), use "
     "format \"mountpoint:speed\" separate disks with whitespace \" \".\n"
     "#* Example: \"/mnt/media:100 /:20 /boot:1\"."},

    {"net_download",
     "#* Set fixed values for network graphs in Mebibits. Is only used if "
     "net_auto is also set to False."},

    {"net_upload", ""},

    {"net_auto",
     "#* Use network graphs auto rescaling mode, ignores any values set above "
     "and rescales down to 10 Kibibytes at the lowest."},

    {"net_sync",
     "#* Sync the auto scaling for download and upload to whichever currently "
    ...
    {"selected_battery",
     "#* Which battery to use if multiple are present. \"Auto\" for auto "
     "detection."},

    {"log_level",
     "#* Set loglevel for \"~/.config/btop/btop.log\" levels are: \"ERROR\" "
     "\"WARNING\" \"INFO\" \"DEBUG\".\n"
     "#* The level set includes all lower levels, i.e. \"DEBUG\" will show all "
     "logging info."}};
...
unordered_flat_map<string, bool> bools = {
    ...
    {"rounded_corners", true},  {"proc_reversed", false},
    {"proc_tree", false},       {"proc_colors", true},
    {"proc_gradient", true},    {"proc_per_core", false},
    {"proc_mem_bytes", true},   {"proc_info_smaps", false},
    {"proc_left", false},       {"proc_filter_kernel", false},
    {"cpu_invert_lower", true}, {"cpu_single_graph", false},
    {"cpu_bottom", false},      {"show_uptime", true},
    {"check_temp", true},       {"show_coretemp", true},
    {"show_cpu_freq", true},    {"background_update", true},
    {"mem_graphs", true},       {"mem_below_net", false},
    {"zfs_arc_cached", true},   {"show_swap", true},
    {"swap_disk", true},        {"show_disks", true},
    {"only_physical", true},    {"use_fstab", true},
    {"show_io_stat", true},     {"io_mode", false},
    {"base_10_sizes", false},   {"io_graph_combined", false},
    {"net_auto", true},         {"net_sync", false},
    {"show_battery", true},     {"vim_keys", false},
    {"tty_mode", false},        {"disk_free_priv", false},
    {"force_tty", false},       {"lowcolor", false},
    {"show_detailed", false},   {"proc_filtering", false},
};
unordered_flat_map<string, bool> boolsTmp;

unordered_flat_map<string, int> ints = {
    {"update_ms", 2000},  {"net_download", 100},     {"net_upload", 100},
    {"detailed_pid", 0},  {"selected_pid", 0},       {"proc_start", 0},
    {"proc_selected", 0}, {"proc_last_selected", 0},
};
unordered_flat_map<string, int> intsTmp;

bool _locked(const string& name) {
    atomic_wait(writelock, true);
    if (not write_new and rng::find_if(descriptions, [&name](const auto& a) {
                              return a.at(0) == name;
                          }) != descriptions.end())
        write_new = true;
    return locked.load();
...
bool stringValid(const string& name, const string& value) {
    ...
    else if (name == "graph_symbol" and
             not v_contains(valid_graph_symbols, value))
        validError = "Invalid graph symbol identifier: " + value;

    else if (name.starts_with("graph_symbol_") and
             (value != "default" and
              not v_contains(valid_graph_symbols, value)))
        validError =
            "Invalid graph symbol identifier for" + name + ": " + value;

    else if (name == "shown_boxes" and not value.empty() and
             not check_boxes(value))
...
void unlock() {
    ...
    try {
        ...
        Global::exit_error_msg =
            "Exception during Config::unlock() : " + (string)e.what();
        clean_quit(1);
    }
...
void load(const fs::path& conf_file, vector<string>& load_warnings) {
    ...
    if (cread.good()) {
        ...
        for (auto& n : descriptions) valid_names.push_back(n[0]);
        if (string v_string;
            cread.peek() != '#' or (getline(cread, v_string, '\n') and
                                    not s_contains(v_string, Global::Version)))
            write_new = true;
        while (not cread.eof()) {
            cread >> std::ws;
            if (cread.peek() == '#') {
            ...
            if (bools.contains(name)) {
                ...
                if (not isbool(value))
                    load_warnings.push_back(
                        "Got an invalid bool value for config name: " + name);
                else
